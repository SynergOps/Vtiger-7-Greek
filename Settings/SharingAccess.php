<?php
/*+***********************************************************************************
 * The contents of this file are subject to the vtiger CRM Public License Version 1.0
 * ("License"); You may not use this file except in compliance with the License
 * The Original Code is:  vtiger CRM Open Source
 * The Initial Developer of the Original Code is vtiger.
 * Portions created by vtiger are Copyright (C) vtiger.
 * All Rights Reserved.
 *************************************************************************************/
$languageStrings = array(
	'SharingAccess' => 'Κανόνες Διαμοιρασμού',
	'Accounts' => 'Εταιρείες & Πελάτες',
	'LBL_ADD_CUSTOM_RULE' => 'Προσθήκη Προσαρμοσμένου Κανόνα',
	'Read Only' => 'Ανάγνωση μόνο',
	'Read Write' => 'Ανάγνωση/Εγγραφή',
	'LBL_ADD_CUSTOM_RULE_TO' => 'Προσθήκη Προσαρμοσμένου Κανόνα σε',
	'LBL_CAN_ACCESSED_BY' => 'Μπορεί να προσπελαστεί από',
	'LBL_PRIVILEGES' => 'Δικαιώματα',
	'LBL_SHARING_RULE' =>  'Κανόνες Διαμοιρασμού',
	'LBL_RULE_NO' => 'Αρ. Κανόνα',
	'LBL_MODULE' => 'Ενότητα',
	'LBL_ADVANCED_SHARING_RULES' => 'Σύνθετοι Κανόνες Διαμοιρασμού',
	'LBL_WITH_PERMISSIONS' => 'Με Δικαιώματα',
	'LBL_APPLY_NEW_SHARING_RULES' => 'Εφαρμογή των νέων κανόνων Διαμοιρασμού',
	'LBL_READ' => 'Ανάγνωση',
	'LBL_READ_WRITE' => 'Ανάγνωση και Εγγραφή',
	'LBL_CUSTOM_ACCESS_MESG' => 'Δεν έχει οριστεί κανένας προσαρμοσμένος κανόνας',
	'SINGLE_Groups' => 'Ομάδα',
	'SINGLE_Roles' => 'Ρόλος',
	'SINGLE_RoleAndSubordinates' => 'Ρόλοι και Δευτερεύοντες',
);

$jsLanguageStrings = array(
	'JS_CUSTOM_RULE_SAVED_SUCCESSFULLY' => 'Προσαρμοσμένος κανόνας διαμοιρασμού αποθηκεύτηκε με επιτυχία',
	'JS_SELECT_ANY_OTHER_ACCESSING_USER' => 'Επιλέξτε οποιονδήποτε άλλο χρήστη που έχει πρόσβαση',
	'JS_NEW_SHARING_RULES_APPLIED_SUCCESSFULLY' => 'Νέοι κανόνες διαμοιρασμού αποθηκεύτηκαν με επιτυχία',
	'JS_DEPENDENT_PRIVILEGES_SHOULD_CHANGE' => 'Η πρόσβαση σε Ευκαιρίες, Αιτήματα, Προσφορές, Παραγγελίες Πωλήσεων & Τιμολόγια πρέπει να τεθούν σε Ιδιωτικό όταν η Πρόσβαση Εταιρειών έχει τεθεί σε Ιδιωτικό',
);
