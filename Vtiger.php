<?php
/*+***********************************************************************************
 * The contents of this file are subject to the vtiger CRM Public License Version 1.1
 * ("License"); You may not use this file except in compliance with the License
 * The Original Code is: vtiger CRM Open Source
 * The Initial Developer of the Original Code is vtiger.
 * Portions created by vtiger are Copyright (C) vtiger.
 * All Rights Reserved.
 *************************************************************************************/
$languageStrings = array(

	// Footer
	'APPTITLE' => 'vtiger CRM',
	'POWEREDBY'=> 'Βασισμένο στο vtiger CRM',
	'LBL_READ_LICENSE' => 'Άδεια Χρήσης',
	'LBL_PRIVACY_POLICY' => 'Πολιτική Απορρήτου',

	// Common Actions
	'LBL_NEW' => 'Νέο',
	'LBL_LINKED' => 'Συνδεδεμένο',
	'LBL_SELECT_ALL' => 'Επιλογή όλων',
	'LBL_SELECT_TO_LOAD_LIST' => 'Επιλέξτε φόρτωμα Λίστας',
	'LBL_CREATING_NEW' => 'Δημιουργία Νέας',
	'LBL_ADDING_NEW' => 'Προσθήκη νέας',
	'LBL_EDITING' => 'Επεξεργασία',
	'LBL_SAVE' => 'Αποθήκευση',
	'LBL_CANCEL' => 'Ακύρωση',
	'LBL_BACK' => 'Πίσω',
	'LBL_IMPORT' => 'Εισαγωγή',
	'LBL_EXPORT' => 'Εξαγωγή',
	'LBL_FIND_DUPLICATES' => 'Βρείτε διπλότυπα',
	'LBL_MORE' => 'Περισσότερα',
	'LBL_LESS' => 'Λιγότερα',
	'LBL_ACTIONS' => 'Ενέργειες',
	'LBL_ADD' => 'Προσθήκη',
	'LBL_EDIT' => 'Επεξεργασία',
	'LBL_DELETE' => 'Διαγραφή',
	'LBL_COMMENT' => 'Σχόλια',
	'LBL_STARRED' => 'Μην ακολουθείτε',
	'LBL_NOT_STARRED' => 'Κλικ για να ακολουθήσετε',
	'LBL_QUICK_VIEW' => 'Σύντομη Προβολή',
	'LBL_NOTIFICATION_CENTER' => 'Κέντρο ειδοποιήσεων',
	'LBL_NOTIFICATION' => 'Ειδοποίηση',
	'LBL_STARRED_RECORD_TO' => '%s έως %s',
	'LBL_STARRED_RECORD_UPDATED' => '%s επικαιροποίηση %s',
	'LBL_SETTINGS'=> 'Ρυθμίσεις',
	'LBL_ADD_COMMENT' => 'Αφήστε Σχόλιο',
	'LBL_EDIT_FIELDS' => '%s Πεδία & Διάταξη',
	'LBL_EDIT_WORKFLOWS' => '%s Ροές εργασίας',
	'LBL_EDIT_PICKLIST_VALUES' => 'Τιμές Λίστας επιλογών',
	'LBL_EDIT_MAILSCANNER' => 'Μετατροπέας Mail',
	'LBL_RECORD_DELETE' => 'Η εγγραφή που προσπαθείτε να προβάλετε έχει διαγραφεί.',
	'LBL_RECORD_NOT_FOUND' => 'Η εγγραφή στην οποία προσπαθείτε να προσπελάσετε δεν βρέθηκε',
	'LBL_EDIT_REASON' => 'Αιτία επεξεργασίας',
	//'LBL_EDIT_CURRENT_FILTER' => 'Επεξεργασία Τρέχον φίλτρου',
	//'LBL_DELETE_CURRENT_FILTER' => 'Διαγραφή Τρέχον φίλτρου',
	'LBL_SELECT' => 'Επιλέξτε',
	'LBL_CLEAR' => 'Καθαρισμός',
	'LBL_YES' => 'Ναι',
	'LBL_NO' => '0',
	'LBL_SHOW_COMPLETE_DETAILS' => 'Πλήρης Λεπτομέρειες',
	'LBL_MINILIST_FIELDS_ARE_NOT_AVAILABLE_FOR_YOU' => 'Συγνώμη! Δεν είναι διαθέσιμα τα πεδία μινιλίστας για εσάς.',
	'LBL_MASS_EDITING' => 'Μαζική Επεξεργασία',
	'LBL_DESELECT_ALL_RECORDS' => 'Αποεπιλογή όλων των εγγραφών σε όλες τις σελίδες',
	'LBL_QUICK_CREATE' => 'Γρήγορη δημιουργία',
	'LBL_QUICK_EDIT' => 'Γρήγορη Επεξεργασία',
	'LBL_SEND_EMAIL' => 'Αποστολή Email',
	//SEND EMAIL translations
	'LBL_ALL_EMAILS' => 'Όλοι οι λογαριασμοί email',
	'LBL_MUTIPLE_EMAIL_SELECT_ONE' => 'Διαλέξτε μία από τις παρακάτω επιλογές',
	//SEND EMAIL translations
	'LBL_GO_TO_FULL_FORM' => 'Μετάβαση σε πλήρης φόρμα',
	'LBL_SEND_SMS' => 'Αποστολή SMS',
	'LBL_FOUND' => 'βρέθηκε',
	'LBL_CLICK_ADD' => 'Κλικ για Προσθήκη',
	'LBL_ADD_MORE_FIELDS' => 'Προσθέστε περισσότερα πεδία',
	'LBL_ADD_NOTE' => 'Προσθήκη Αρχείο',
	'LBL_CREATE_NEW' => 'Δημιουργία Νέου',
	'LBL_ADD_EVENT' => 'Προσθήκη Συμβάντος',
	'LBL_ADD_TASK' => 'Προσθήκη εργασίας',
	'LBL_MARKETING_AND_SALES' => 'MARKETING & ΠΩΛΗΣΕΙΣ',
	'LBL_TOOLS_HEADER' => 'ΕΡΓΑΛΕΊΑ',
	'LBL_SUPPORT' => 'ΥΠΟΣΤΗΡΙΞΗ',
	'LBL_INVENTORY' => 'ΑΠΟΘΗΚΗ',
	'LBL_ANALYTICS' => 'ΣΤΑΤΙΣΤΙΚΑ',
	'LBL_CONTACT' => 'ΔΙΑΧΕΙΡΙΣΗ ΕΠΙΚΟΙΝΩΝΙΑΣ',
	'LBL_HOME' => 'Αρχική',
	'LBL_NO_ATTACHMENTS' => 'Δεν υπάρχουν συνημμένα',
	'LBL_COMPOSE_EMAIL_WARNING' => ' Προειδοποίηση: Το email σας ενδέχεται να καταλήξει στα ανεπιθύμητα του παραλήπτη. Προσθέστε το Vtiger ως εξουσιοδοτημένο αποστολέα για τον τομέα σας. Μάθε περισσότερα',
	// Common Link Labels
	'LBL_REARRANGE_DASHBOARD_TABS' => 'Αναδιάταξη καρτελών',
	'LBL_DONE' => 'Εγινε',
	'LBL_DELETE_DASHBOARD_TAB' =>'Διαγραφή καρτέλας',
	'LBL_DASHBOARD_TAB_ALREADY_EXIST' => 'Το όνομα της καρτέλας υπάρχει ήδη',
	'LBL_TABS_LIMIT_EXCEEDED' => 'Συγνώμη! Δεν είναι δυνατή η προσθήκη περισσότερων από 10 καρτελών',
	'LBL_TODOS' => 'Εργασίες',
	'LBL_LAST_VIEWED_RECORDS' => 'Τελευταίες προβεβλημένες εγγραφές',
	'LBL_DASHBOARD' => 'Πίνακας ελέγχου',
	'LBL_USER_SETTINGS' => 'Ρυθμίσεις Χρήστη',
	'LBL_MY_PREFERENCES' => 'Οι Προτιμήσεις Μου',
	'LBL_SIGN_OUT' => 'Αποσύνδεση',
	'LBL_HELP' => 'Βοήθεια',
	'LBL_DOCUMENTATION' => 'Τεκμηρίωση',
	'LBL_VIDEO_TUTORIAL' => 'Οδηγός χρήσης σε Βίντεο',
	'LBL_FEEDBACK' => 'Παρατηρήσεις',
	'LBL_CHAT_SUPPORT'=>'Chat με την Υποστήριξη',
	'LBL_CRM_SETTINGS' => 'Ρυθμίσεις CRM',
	'LBL_ON_DEMAND_ADMIN' => 'Προσθήκη/Διαγραφή χρηστών',
	'LBL_ANNOUNCEMENT' => 'Ανακοίνωση',
	'LBL_CALENDAR' => 'Ημερολόγιο',
	'LBL_POST' => 'Δημοσίευση',
	'LBL_GUIDER' => 'Οδηγός',
	'LBL_VIDEOS' => 'Βίντεο',
	'LBL_DISPLAY_TYPE' => 'Τύπος Προβολής',
	'LBL_DISPLAY_WIDETYPE' => 'Ευρεία',
	'LBL_DISPLAY_MEDIUMTYPE' => 'Μεσαία',
	'LBL_DISPLAY_NARROWTYPE' => 'Στενής',
	'LBL_TAG_THIS_RECORD' => 'Ετικέτα την εγγραφή',
	'LBL_SAME_TAG_EXISTS' => 'Υπάρχει διπλότυπη ετικέτα',
	'LBL_MAKE_PUBLIC' => 'Κάντε το Δημόσιο',
	'LBL_PAGE' => 'Σελίδα',
	'LBL_LISTVIEW_PAGE_JUMP' => 'Προσπεράστε σελίδα',
	'LBL_LISTVIEW_JUMP_TO' => 'Προσπεράστε στην',
	'LBL_PROFILE_PASSWORD' => 'Προφίλ / Συνθηματικό',
	'LBL_INFO' => 'Πληροφορίες',
	'LBL_CANT_MOVE_FROM_PUBLIC_TO_PRIVATE' => 'Δεν είναι δυνατή η μετάβαση από την Δημόσια ετικέτα στην Ιδιωτική ετικέτα.',
	'LBL_EDIT_TAG'=>'Επεξεργασία Ετικέτας',
	'LBL_NO_TAG_EXISTS' => 'Δεν υπάρχει ετικέτα' ,
	'LBL_RELATED_USERS' => 'Σχετιζόμενοι χρήστες',

	// Common Fields
	'LBL_USER' => 'Χρήστης',
	'LBL_GROUP' => 'Ομάδα',
	'Last Modified By' => 'Τελευταία τροποποίηση από',
	'Created Time' => 'Δημιουργήθηκε στις',
	'Modified Time' => 'Τροποποιήθηκε στις',
	'Description' => 'Περιγραφή',
	'Assigned To'=>'Ανατέθηκε σε/στον',

	//Loading Labels
	'LBL_LOADING' => 'Φόρτωση',
	'LBL_LOADING_WIDGET' => 'Φόρτωση Widget',
	'LBL_LOADING_LISTVIEW_CONTENTS'	=> 'Γίνετε Φόρτωση παρακαλώ περιμένετε',

	// Detail View
	'LBL_HISTORY' => 'Ιστορικό',
	'LBL_UPDATES' => 'Ενημερώσεις',
	'LBL_SHOW_FULL_DETAILS' => 'Εμφάνιση πλήρων λεπτομερειών',
	'LBL_SUMMARY_DETAILS' => 'Περιληπτικά Στοιχεία',
	'LBL_ADD_NEW_COMMENT' =>'Προσθήκη Νέου Σχόλιου',
	'LBL_NO_UPDATES' => 'Καμία Ενημέρωση',
	'LBL_ACTIVITIES' => 'Δραστηριότητες',
	'LBL_NO_PENDING_ACTIVITIES' => 'Καμία εκκρεμής δραστηριότητα',
	'LBL_SUMMARY' => 'Περίληψη',
	'LBL_KEY_FIELD' => 'Βασικό πεδίο',
	'LBL_DETAILS' => 'Λεπτομέρειες',
	'LBL_NAME_EMAIL' => 'Όνομα και Email',

	// Popup window
	'LBL_SEARCH_FOR' => 'Αναζήτηση για',
	'LBL_IN' => 'σε',
	'LBL_TYPE_SEARCH' => 'Πληκτρολογήστε για Αναζήτηση',
	'LBL_SEARCH_BUTTON' => 'Κουμπί Αναζήτησης',
	'LBL_ADVANCE_SEARCH' => 'Σύνθετη',
	'LBL_LOADING_PLEASE_WAIT' => 'Φορτώνει...Παρακαλώ περιμένετε',
	'LBL_PLEASE_SELECT_MODULE' => 'Παρακαλώ επιλέξτε οποιαδήποτε ενότητα για αναζήτηση',

	//DropDown Category
	'LBL_USERS' => 'Χρήστες',
	'LBL_GROUPS' => 'Ομάδες',

	//EditView Form Links
	'LBL_FULL_FORM' => 'Πλήρης Φόρμα',
	'LBL_SIMPLE_FORM' => 'Απλή Φόρμα',

	//Recent Comments
	'LBL_IS_REMOVED' => 'έχει αφαιρεθεί',
	'LBL_ADD_YOUR_COMMENT_HERE' => 'Αφήστε το Σχόλιο σας εδώ....',
	'LBL_REASON_FOR_CHANGING_COMMENT' => 'Αιτία τροποποίησης σχόλιου',
	'LBL_REPLY' => 'Απάντηση',
	'LBL_VIEW_THREAD' => 'Δείτε την συζήτηση',
	'LBL_NO_COMMENTS' => 'Κανένα Σχόλιο',
	'LBL_REPLIES' => 'απαντήσεις',
	'LBL_REPLY_ALL' => 'Απάντηση σε όλα',

	//Translation for all modules
	'Accounts' => 'Εταιρείες',
	'Assets' => 'Περιουσιακά Στοιχεία',
	'Calendar' => 'Ημερολόγιο',
	'Campaigns' => 'Καμπάνιες',
	'Contacts' => 'Πελάτες',
	'Documents' => 'Αρχεία',
	'Leads' => 'Δυνητικοί',
	'ModComments' => 'Σχόλια',
	'Potentials' => 'Ευκαιρίες',
	'PriceBooks' => 'Τιμοκατάλογοι',
	'Products' => 'Προϊόντα',
	'Project' => 'Έργα',
	'ProjectMilestone' => 'Ορόσημα του Έργου',
	'ProjectTask' => 'Εργασίες του Έργου',
	'ServiceContracts' => 'Συμβόλαια Υπηρεσιών',
	'Services' => 'Υπηρεσίες',
	'Vendors' => 'Προμηθευτές',
	'Rss' => 'RSS',
	'Quotes'=>'Προσφορές',
	'PurchaseOrder'=>'Εντολή Αγοράς',
	'SalesOrder'=>'Παραγγελία πώλησης',
	'Invoice'=>'Τιμολόγιο',
	'MailManager' => 'Διαχειριστής Mail',
	'Activities' => 'Δραστηριότητες',
	'Portal' => 'Ιστοσελίδες μας',
	'Email Templates' => 'Πρότυπα Email',
	'Recycle Bin' => 'Κάδος ανακύκλωσης',

	//Basic String Translation
	'LBL_ADD_RECORD' => 'Προσθήκη Εγγραφής',
	'LBL_RECENTLY_MODIFIED' => 'Πρόσφατη Τροποποίηση',
	'LBL_RECORDS_LIST' => 'Λίστα Εγγραφών',
	'LBL_RECORD_SUMMARY' => 'Σύνοψη Ενότητας',
	'LBL_RECORD_SUMMARY' => 'Λεπτομέρειες Ενότητας',
	'LBL_ACCESS_DENIED_FOR' => 'Απαγορεύεται η πρόσβαση για',
	'LBL_GO_BACK' => 'Πίσω',
	'LBL_NO_DATA_AVAILABLE' => 'Δεν υπάρχουν διαθέσιμα δεδομένα',

	// Global Search
	'LBL_GLOBAL_SEARCH' => 'Πληκτρολογήστε μία λέξη και πατήστε enter',
	'LBL_ALL_RECORDS' => 'Όλες οι εγγραφές',
	'LBL_NO_RECORDS_FOUND' => 'Δεν Βρέθηκε καμία εγγραφή',

	//Filters & Advance Filter
	'LBL_AND' => 'και',
	'LBL_OR' => 'ή',
	'LBL_APPLBL_NONEROVE' => 'Έγκριση',
	'LBL_DENY' => 'Άρνηση',
	'LBL_EQUALS' => 'ίσο με',
	'LBL_NOT_EQUAL_TO' => 'δεν είναι ίσο με',
	'LBL_STARTS_WITH' => 'ξεκινάει με',
	'LBL_ENDS_WITH' => 'τελειώνει με',
	'LBL_CONTAINS' => 'περιέχει',
	'LBL_DOES_NOT_CONTAIN' => 'δεν περιέχει',
	'LBL_LESS_THAN' => 'μικρότερο από',
	'LBL_GREATER_THAN' => 'μεγαλύτερο από',
	'LBL_LESS_THAN_OR_EQUAL' => 'μικρότερο ή ίσο',
	'LBL_GREATER_OR_EQUAL' => 'μεγαλύτερο ή ίσο',
	'LBL_BEFORE' => 'πριν',
	'LBL_AFTER' => 'μετά',
	'LBL_BETWEEN' => 'μεταξύ',
	'LBL_SEARCH' => 'Αναζήτηση',
	'LBL_SEARCH_IN' => 'Αναζήτηση σε',
	'LBL_ADD_CONDITION' => 'Προσθήκη Συνθήκης',
	'LBL_ADD_GROUP' => 'Προσθήκη ομάδας κατάστασης',
	'LBL_FILTER_CONDITIONS' => 'Συνθήκες φίλτρων',
	'LBL_ALL_CONDITIONS' => 'Όλες οι συνθήκες',
	'LBL_ALL_CONDITIONS_DESC' => 'Όλες οι συνθήκες πρέπει να ικανοποιούνται',
	'LBL_ANY_CONDITIONS' => 'Οποιαδήποτε συνθήκη',
	'LBL_ANY_CONDITIONS_DESC' => 'Τουλάχιστον μία συνθήκη πρέπει να ικανοποιείται',
	'LBL_SELECT_FIELD' => 'Επιλέξτε Πεδίο',
	'LBL_SELECT_MODULE' => 'Επιλογή Ενότητας',
	'LBL_CREATE_NEW_FILTER' => 'Δημιουργία Νέου Φίλτρου',
	'All' => 'Όλα',
	'Others' => 'Λοιπά',
	'Pending' => 'Εκκρεμούν',
	'Public' => 'Δημόσια',

	//Advance Search
	'LBL_SAVE_FILTER' => 'Αποθήκευση Φίλτρου',
	'LBL_SAVE_MODIFY_FILTER' => 'Αποθήκευση/Τροποποίηση Αναζήτησης',
	'LBL_SEARCH_RESULTS' => 'Αποτελέσματα Αναζήτησης',
	'LBL_SAVE_AS_FILTER' => 'Αποθήκευση ως Φίλτρο',

	//Exception String Translation
	'LBL_NOT_ACCESSIBLE' => 'Άρνηση Πρόσβασης',

	//Line Items
	'LBL_ITEM_DETAILS'=> 'Λεπτομέρειες Αντικειμένου',
	'LBL_CURRENCY' => ' Συνάλλαγμα',
	'LBL_TAX_MODE' => 'Φορολογική λειτουργία',
	'LBL_INDIVIDUAL' => 'Ατομικό',
	'LBL_GROUP' => 'Ομάδα',
	'LBL_TOOLS' => 'Εργαλεία',
	'LBL_ITEM_NAME' => 'Όνομα Αντικειμένου',
	'LBL_QTY_IN_STOCK' => 'Απόθεμα',
	'LBL_QTY' => 'Ποσότητα',
	'LBL_LIST_PRICE' => 'Τιμή Πώλησης',
	'LBL_TOTAL' => 'Σύνολο',
	'LBL_NET_PRICE' => 'Καθαρή Αξία',
	'LBL_DISCOUNT' => 'Έκπτωση',
	'LBL_TOTAL_AFTER_DISCOUNT' => 'Σύνολο Μετά την Έκπτωση',
	'LBL_TAX' => 'Φόρος',
	'LBL_ADD_PRODUCT' => 'Προσθήκη προϊόντος',
	'LBL_ADD_SERVICE' => 'Προσθήκη Υπηρεσίας',
	'LBL_LI_ADD_PRODUCT' => 'Προϊόν',
	'LBL_LI_ADD_SERVICE' => 'Υπηρεσία',
	'LBL_ITEMS_TOTAL' => 'Σύνολο Αντικειμένων',
	'LBL_SHIPPING_AND_HANDLING_CHARGES' => 'Έξοδα αποστολής και μεταφοράς',
	'LBL_PRE_TAX_TOTAL' => 'Σύνολο προ Φόρου',
	'LBL_TAX_FOR_SHIPPING_AND_HANDLING' => 'Φόρος αποστολής και μεταφοράς',
	'LBL_SET_SHIPPING_AND_HANDLING_TAXES_FOR' => 'Ορίστε ΦΠΑ για Αποστολή και Μεταφορά',
	'LBL_ADJUSTMENT' => 'Προσαρμογή',
	'LBL_DEDUCT' => 'Αφαίρεση',
	'LBL_GRAND_TOTAL' => 'Σύνολο',
	'LBL_ZERO_DISCOUNT' => 'Μηδενική Έκπτωση',
	'LBL_OF_PRICE' => 'Τιμή',
	'LBL_DIRECT_PRICE_REDUCTION' => 'Απευθείας Μείωση Τιμής',
	'LBL_SET_DISCOUNT_FOR' => 'Ορίστε Έκπτωση για',
	'LBL_STOCK_NOT_ENOUGH' => 'Μη Επαρκές Απόθεμα',
	'LBL_MAX_QTY_SELECT' => 'Η Μέγιστη Τιμή είναι',
	'LBL_DRAG' => 'Σύρτε',
	'LBL_TOTAL_TAX_AMOUNT' => 'Συνολικό ποσό Φόρου',
	'LBL_DIRECT_AMOUNT_DISCOUNT' => 'Έκπτωση άμεσου ποσού',
	'LBL_FINAL_DISCOUNT_AMOUNT' => 'Τελικό Ποσό Έκπτωσης',
	'LBL_MORE_CURRENCIES' => 'Περισσότερες Ισοτιμίες',
	'LBL_SET_TAX_FOR' => 'Ορισμός Φόρου Για',
	'LBL_GROUP_TAX' => 'Φόρος Ομίλου',
	'LBL_BILLING_ADDRESS_FROM' => 'Αντιγραφή Διεύθυνσης Πληρωμής από',
	'LBL_SHIPPING_ADDRESS_FROM' => 'Αντιγραφή Διεύθυνσης Αποστολής από',
	'LBL_COPY_SHIPPING_ADDRESS' => 'Διεύθυνση Αποστολής',
	'LBL_COPY_BILLING_ADDRESS' => 'Διεύθυνση Χρέωσης',
	'LBL_OVERALL_DISCOUNT' => 'Συνολική έκπτωση',
	'LBL_CHARGES' => 'Χρεώσεις',
	'LBL_CHARGES_TOTAL' => 'Συνολικές χρεώσεις',
	'LBL_TAXES_ON_CHARGES' => 'Φόροι επί των χρεώσεων',
	'LBL_DEDUCTED_TAXES' => 'Αφαίρεση Φόρων',
	'LBL_DEDUCTED_TAXES_TOTAL' => 'Αφαίρεση Φόρων - Σύνολο',
	'LBL_DEFAULT' => 'Προκαθορισμένο',
	'LBL_ITEM' => 'Αντικείμενο',
	'LBL_ADD_DASHBOARD' =>'Προσθήκη Πίνακα',

	//DetailView Actions
	'LBL_CREATE' => 'Δημιουργία',
	'LBL_GENERATE' => 'Παραγωγή',
	'LBL_DUPLICATE' => 'Διπλότυπο',
	'Add Note' => 'Προσθήκη Αρχείου',

	//Dashboard String
	'LBL_ADD_WIDGET' => 'Προσθήκη Widget',
	'LBL_COMMENTS' => 'Σχόλια',
	'LBL_REFRESH' => 'Ανανέωση',
	'LBL_CLOSE' => 'Κλείσιμο',
	'LBL_ALL' => 'Όλα',
	'LBL_UPDATED' => 'ενημερωμένα',
	'LBL_VIA_TASK' => 'μέσω εργασίας',
	'LBL_form' => 'από',
	'LBL_FROM' => 'Από',
	'LBL_TO' => 'Προς',
	'LBL_to' => 'προς',
	'LBL_ON' => 'στις',
	'LBL_OF' => 'του',
	'LBL_BY' => 'από',
	'LBL_ADDED' => 'προστέθηκε',
	'LBL_FOR' => 'για',
	'LBL_CREATED' => 'δημιουργήθηκε',
	'LBL_DELETED' => 'διαγράφτηκε',
	'LBL_RESTORED' => 'επαναφέρθηκε',
	'LBL_COMMENTED' => 'σχολιάστηκε',
	'LBL_REMOVED' => 'αφαιρέθηκε',
	'LBL_REMOVE' => 'αφαίρεση',
	'LBL_AT' => 'στις',
	'LBL_MINE' => 'Δικά μου',
	'LBL_ME' => 'Εγώ',
	'History' => 'Ιστορικό ενεργειών',
	'Upcoming Tasks' => 'Επόμενες εργασίες',
	'LBL_YEAR' => 'έτος',
	'LBL_YEARS' => 'έτη',
	'LBL_MONTH' => 'μήνας',
	'LBL_MONTHS' => 'μήνες',
	'LBL_DAY' => 'ημέρα',
	'LBL_DAYS' => 'ημέρες',
	'LBL_DAY(S)' => 'ημέρα(ες)',
	'LBL_HOUR' => 'ώρα',
	'LBL_HOURS' => 'ώρες',
	'LBL_HOUR(S)' => 'ώρα(ες)',
	'LBL_MINUTE' => 'λεπτό',
	'LBL_MINUTES' => 'λεπτά',
	'LBL_SECOND' => 'δευτερόλεπτο',
	'LBL_SECONDS' => 'δευτερόλεπτα',
	'LBL_JUSTNOW' => 'μόλις τώρα',
	'LBL_CUSTOM_INFORMATION' => 'Προσαρμοσμένες πληροφορίες',
	'LBL_NO_RECENT_UPDATES' => 'Καμία πρόσφατη ενημέρωση',
	'LBL_NO_DATA' => 'Χωρίς δεδομένα',
	'LBL_PERMISSION_DENIED' => 'Άρνηση Έγκρισης',
	'LBL_HANDLER_NOT_FOUND' => 'δεν βρέθηκε το διαχειριστικό',
	'LBL_FILTER' => 'Φίλτρο',
	'LBL_DUE' => 'μέσα σε',
	'LBL_DAYS' => 'ημέρες',
	'LBL_COMPLETED' => 'ολοκληρώθηκαν',
	'LBL_AGO' => 'πριν από',
	'LBL_CHANGED' => 'αλλάχθηκαν',
	'SINGLE_Users' => 'Χρήστης',
	'LBL_MATCHED_THIS_CRITERIA'=> 'αντιστοιχούν αυτά τα κριτήρια',
	'LBL_NO_SCHEDULED_ACTIVITIES' => 'Καμία προγραμματισμένη δραστηριότητα',
	'LBL_NO_OVERDUE_ACTIVITIES' => 'Καμία εκπρόθεσμη δραστηριότητα',
	'LBL_NO_UPDATES_OR_COMMENTS' => 'Καμία ενημέρωση η σχόλια',
	'LBL_MINI_LIST' => 'Προσαρμοσμένη Λίστα',
	'LBL_NOTEPAD' => 'Σημειωματάριο',
	'LBL_NOTEPAD_NAME' => 'Όνομα Σημειωματάριου',
	'LBL_NOTEPAD_CONTENT' => 'Περιεχόμενο Σημειωματάριου',
	'LBL_LAST_SAVED_ON' => 'Τελευταία Αποθήκευση στις',
	'Mini List' => 'Προσαρμοσμένη Λίστα',
	'Notebook' => 'Σημειωματάριο',
	'LBL_RESULT_FOR_THE_TAG' => 'Αποτέλεσμα για την ετικέτα',
	'LBL_ALL_USERS' => 'Όλοι οι χρήστες',
	'LBL_BOTH' => 'Και τα δυο',
	'LBL_SHOW' => 'Προβολή',
	'LBL_MY' => 'Μου',
	'LBL_SELECT_DATE_RANGE' => 'Επιλέξτε εύρος ημερομηνιών',

	// Basic Strings- custom view
	'LBL_VIEW_NAME' => 'Όνομα Λίστας',
	'LBL_CREATE_VIEW' => 'Δημιουργία νέας προβολής',
	'LBL_BASIC_DETAILS' => 'Βασικές πληροφορίες',
	'LBL_CHOOSE_COLUMNS' => 'Επιλέξτε στήλες και διάταξη',
	'LBL_MAX_NUMBER_FILTER_COLUMNS' => 'Μέγιστο 15',
	'LBL_FILTER_ON_DATE' => 'Φιλτράρισμα ημερομηνίας',
	'LBL_CHOOSE_FILTER_CONDITIONS' => 'Επιλέξτε συνθήκες φίλτρων',
	'LBL_ADD_CONDITION' => 'Προσθήκη Συνθήκης',
	'LBL_SET_AS_DEFAULT' => 'Ορισμός ως Προεπιλογή',
	'LBL_LIST_IN_METRICS' => 'Εμφάνιση στους Μετρητές',
	'LBL_SET_AS_PUBLIC' => ' Ορισμός ως Δημόσιο',
	'LBL_AND' => 'και',
	'LBL_OR' => 'ή',
	'LBL_ADD_MORE_COLUMNS' => 'Προσθήκη περισσότερων στηλών',
	'LBL_ADD_GROUP' => 'Προσθήκη Ομάδος',

	//DateFilter Conditions
	'LBL_CUSTOM' => 'Προσαρμοσμένο',
	'LBL_PREVIOUS_FY' => 'Προηγούμενο FY',
	'LBL_CURRENT_FY' => 'Τρέχων FY',
	'LBL_NEXT_FY' => 'Επόμενο FY',
	'LBL_PREVIOUS_FQ' => 'Προηγούμενο FQ',
	'LBL_CURRENT_FQ' => 'Τρέχων FQ',
	'LBL_NEXT_FQ' => 'Επόμενο FQ',
	'LBL_YESTERDAY' => 'Χθες',
	'LBL_TODAY' => 'Σήμερα',
	'LBL_TOMORROW' => 'Αύριο',
	'LBL_LAST_WEEK' => 'Προηγούμενη Εβδομάδα',
	'LBL_CURRENT_WEEK' => 'Τρέχων Εβδομάδα',
	'LBL_NEXT_WEEK' => 'Επόμενη Εβδομάδα',
	'LBL_LAST_MONTH' => 'Προηγούμενος Μήνας',
	'LBL_CURRENT_MONTH' => 'Τρέχων Μήνας',
	'LBL_NEXT_MONTH' => 'Επόμενος Μήνας',
	'LBL_LAST_7_DAYS' => 'Τελευταίες 7 ημέρες',
	'LBL_LAST_14_DAYS' => 'Τελευταίες 14 ημέρες',
	'LBL_LAST_30_DAYS' => 'Τελευταίες 30 ημέρες',
	'LBL_LAST_60_DAYS' => 'Τελευταίες 60 ημέρες',
	'LBL_LAST_90_DAYS' => 'Τελευταίες 90 ημέρες',
	'LBL_LAST_120_DAYS' => 'Τελευταίες 120 ημέρες',
	'LBL_NEXT_30_DAYS' => 'Επόμενες 30 ημέρες',
	'LBL_NEXT_60_DAYS' => 'Επόμενες 60 ημέρες',
	'LBL_NEXT_90_DAYS' => 'Επόμενες 90 ημέρες',
	'LBL_NEXT_120_DAYS' => 'Επόμενες 120 ημέρες',

	//advanced Filter Options
	'LBL_NONE' => 'κανένα',
	'LBL_EQUALS' => 'ίσο με',
	'LBL_NOT_EQUAL_TO' => 'δεν είναι ίσο με',
	'LBL_STARTS_WITH' => 'ξεκινάει με',
	'LBL_ENDS_WITH' => 'τελειώνει με',
	'LBL_CONTAINS' => 'περιέχει',
	'LBL_DOES_NOT_CONTAIN' => 'δεν περιέχει',
	'LBL_LESS_THAN' => 'μικρότερο από',
	'LBL_GREATER_THAN' => 'μεγαλύτερο από',
	'LBL_LESS_THAN_OR_EQUAL' => 'μικρότερο ή ίσο',
	'LBL_GREATER_OR_EQUAL' => 'μεγαλύτερο ή ίσο',
	'LBL_BEFORE' => 'πριν',
	'LBL_AFTER' => 'μετά',
	'LBL_BETWEEN' => 'μεταξύ',
	'LBL_IS_EMPTY'=> 'κενό',
	'LBL_IS_NOT_EMPTY' => 'δεν είναι άδειο',
	'LBL_APPROVE' => 'Έγκριση',
	'LBL_HAS_CHANGED' => 'έχει αλλάξει',
	'LBL_HAS_CHANGED_TO' => 'έχει αλλάξει σε',
	'LBL_HAS_CHANGED_FROM' => 'έχει αλλάξει από',
	'LBL_LESS_THAN_OR_EQUAL_TO' => 'μικρότερο ή ίσο με',
	'LBL_GREATER_THAN_OR_EQUAL_TO' => 'μεγαλύτερο ή ίσο με',
	'LBL_IS_NOT' => 'δεν είναι',
	'LBL_IS_ADDED' => 'έχει προστεθεί',
	'LBL_IS_TODAY' => 'είναι σήμερα',
	'LBL_IS_TOMORROW' => 'είναι αύριο',
	'LBL_IS_YESTERDAY' => 'είναι χτες',
	'LBL_LESS_THAN_DAYS_LATER' => 'λιγότερο από ημέρες αργότερα',
	'LBL_MORE_THAN_DAYS_LATER' => 'περισσότερο από ημέρες αργότερα',

	//Detail view summary widgets
	'LBL_OWNER' => 'Κάτοχος',
	'LBL_CREATED_ON' => 'Δημιουργήθηκε στις',
	'LBL_MODIFIED_ON' => 'Τροποποιήθηκε στις',
	'LBL_MODIFIED' => 'Τροποποιήθηκε',
	'LBL_COMMENTED' => 'Σχολιάστηκε',

	//calender set-reminder
	'LBL_BEFORE_EVENT' => ' Πριν από το Συμβάν',

	//Dashboard widgets
	'History' => 'Ιστορικό',
	'Upcoming Activities' => 'Προσεχείς Δραστηριότητες',
	'Overdue Activities' => 'Εκπρόθεσμες Δραστηριότητες',
	'Funnel' => 'Χωνί',
	'Potentials by Stage' => 'Ευκαιρίες Ανά Στάδιο',
	'Pipelined Amount' => 'Pipeline Πωλήσεων',
	'Total Revenue' => 'Συνολικά Έσοδα',
	'Top Potentials' => 'Κορυφαίες Ευκαιρίες',
	'Forecast' => 'Πρόγνωση',
	'Leads Created' => 'Εγγεγραμμένες Προοπτικές',
	'Leads by Status' => 'Προοπτικές ανά Κατάσταση',
	'Leads by Source' => 'Προοπτικές ανά Πηγή ',
	'Leads by Industry' => 'Προοπτικές ανά Κλάδο',
	'Tickets by Status' => 'Δελτία ανά Κατάσταση',
	'Open Tickets' => 'Ανοιχτά Δελτία',
	'Funnel Amount' => 'Συνολικό Ποσό ανά Επίπεδο Πωλήσεων',
	'Key Metrics' => 'Βασικές Μετρήσεις',
	'Mini List' => 'Προσαρμοσμένη Λίστα',
	'Tag Cloud' => 'Ετικετοσύννεφο',
	'Notebook' => 'Σημειωματάριο',

	//Exports strings
	'LBL_EXPORT_ALL_DATA' => 'Εξαγωγή Όλων των Δεδομένων',
	'LBL_EXPORT_DATA_IN_CURRENT_PAGE' => 'Εξαγωγή Δεδομένων σε Αυτή την Σελίδα',
	'LBL_EXPORT_SELECTED_RECORDS' => 'Εξαγωγή των Επιλεγμένων εγγραφών',
	'LBL_EXPORT_RECORDS' => 'Εξαγωγή εγγραφών',
	'LBL_NO_RECORD_SELECTED' => 'Καμία εγγραφή δεν έχει Επιλεχθεί',
	'LBL_CAN_NOT_REMOVE_DEFAULT_WIDGET' => 'Το προεπιλεγμένο widget δεν αφαιρείται',
	'LBL_CUSTOM_VIEW_NAME_DUPLICATES_EXIST' => 'Αυτό το φίλτρο υπάρχει ήδη',
	'LBL_TYPE_AND_SEARCH' => 'Πληκτρολόγηση και αναζήτηση',
		'LBL_EXPORT_CURRENCY_TOOLTIP_TEXT' => 'Για την εξαγωγή τιμών συναλλάγματος στα στοιχεία γραμμής, το Vtiger θα χρησιμοποιήσει αυτήν τη ρύθμιση για εξαγωγή ως αυτό το συνάλλαγμα. Οποιεσδήποτε τιμές συναλλάγματος εκτός των στοιχείων γραμμής πινάκων, θα εξάγονται όπως με βάση το προτιμώμενο συνάλλαγμα του χρήστη (Οι προτιμήσεις μου)',
		'LBL_EXPORT_LINEITEM_CURRENCY'=>'Επιλέξτε συνάλλαγμα (για τιμές στοιχείων γραμμής)',
		'LBL_EXPORT_USER_CURRENCY' =>'Το προτιμώμενο συνάλαγμα μου',
		'LBL_EXPORT_RECORD_CURRENCY'=>'Νόμισμα που καθορίζεται στην εγγραφή',
	'LBL_EXPORT_FORMAT' => 'Μορφή Εξαγωγής',
	'LBL_EXPORT_DATA' => 'Εξαγωγή Δεδομένων',

	'LBL_CUSTOMIZE' => 'Προσαρμογή',
	'LBL_ADD_MANAGE_MODULES' => 'Ενότητες',
	'LBL_CUSTOMIZE_MAIN_MENU' => 'Αριστερό Μενού',
	'LBL_LABELS' => 'Όνομα πεδίων',
	'LBL_WORKFLOWS' => 'Ροές διεργασιών',
	'LBL_PICKLIST' => 'Λίστε επιλογών',

	// Added Common Keys for all modules
	'Account Name' => 'Όνομα Εταιρείας',
	'Add Comment' => 'Προσθήκη Σχόλιου',
	'Adjustment' => 'Προσαρμογή',
	'Annual Revenue' => 'Ετήσια Έσοδα',
	'Apparel'=>'Ενδύματα',
	'Banking'=>'Τραπεζικό',
	'Billing Address' => 'Διεύθυνση Χρέωσης ',
	'Billing City' => 'Πόλη Χρέωσης',
	'Billing Code' => 'Ταχ. Κώδικας Χρέωσης',
	'Billing Country' => 'Χώρα Πληρωμής',
	'Billing Po Box' => 'Τ.Θ. Χρέωσης',
	'Billing State' => 'Νομός Χρέωσης',
	'Biotechnology'=>'Βιοτεχνολογία',

	//for SO PO and Quotes
	'FedEx'=>'FedEx',
	'UPS'=>'UPS',
	'USPS'=>'USPS',
	'DHL'=>'DHL',
	'BlueDart'=>'BlueDart',
	'Carrier' => 'Μεταφορέας',
	'Category'=>'Κατηγορία',
	'Chemicals'=>'Χημικά',
	'City' => 'Πόλη',

	'Cold Call'=>'Τηλεφωνική Επαφή',
	'Existing Customer'=>'Υφιστάμενος Πελάτης',
	'Self Generated'=>'Αυτοδημιούργητος',
	'Employee'=>'Υπάλληλος',
	'Partner'=>'Συνέταιρος',
	'Public Relations'=>'Δημόσιες Σχέσεις',
	'Direct Mail'=>'Απευθείας Email',
	'Conference'=>'Συνέδριο',
	'Trade Show'=>'Εμπορική Έκθεση',
	'Web Site'=>'Ιστοσελίδα',
	'Word of mouth'=>'Δια Στόματος',
	'Other'=>'Άλλο',

	//for leads and accounts
	'--None--'=>'--Κανένα--',
	'Acquired'=>'Αποκτήθηκε',
	'Active'=>'Ενεργός',
	'Market Failed'=>'Η αγορά απέτυχε',
	'Project Cancelled'=>'Το Έργο Ματαιώθηκε',
	'Shutdown'=>'Έκλεισε',
	'Apparel'=>'Ενδύματα',
	'Banking'=>'Τραπεζικός',
	'Biotechnology'=>'Βιοτεχνολογία',
	'Chemicals'=>'Χημικά',
	'Communications'=>'Επικοινωνίες',
	'Construction'=>'Κατασκευές',
	'Consulting'=>'Συμβουλευτική',
	'Education'=>'Εκπαίδευση',
	'Electronics'=>'Ηλεκτρονικά',
	'Energy'=>'Ενέργεια',
	'Engineering'=>'Μηχανολογικά',
	'Entertainment'=>'Διασκέδαση',
	'Environmental'=>'Περιβαλλοντολογική',
	'Finance'=>'Οικονομική',
	'Food & Beverage'=>'Φαγητό και Αναψυκτικά',
	'Government'=>'Κρατική',
	'Healthcare'=>'Υγεία',
	'Hospitality'=>'Φιλοξενία',
	'Insurance'=>'Ασφάλιση',
	'Machinery'=>'Επιστήμες Μηχανικών',
	'Manufacturing'=>'Βιομηχανία',
	'Media'=>'Μ.Μ.Ε',
	'Not For Profit'=>'Μη Κερδοσκοπική',
	'Recreation'=>'Μεταποίησης',
	'Retail'=>'Λιανικής',
	'Shipping'=>'Μεταφορική',
	'Technology'=>'Τεχνολογίας',
	'Telecommunications'=>'Τηλεπικοινωνιών',
	'Transportation'=>'Ταξίδια',
	'Utilities'=>'Παροχών',

	'Contact Name' => 'Όνομα Πελάτη',
	'Conversion Rate' => 'Ποσοστό μετατροπής',

	'Shipping Address' => 'Διεύθυνση Αποστολής',
	'Shipping City' => 'Πόλη Αποστολής',
	'Shipping State' => 'Νομός Αποστολής',
	'Shipping Code' => 'Κωδικός Αποστολής',
	'Shipping Country' => 'Χώρα Αποστολής',
	'Shipping Po Box' => 'Τ.Θ.',
	'Country' => 'Χώρα',
	'Description' => 'Περιγραφή',

	'Created'=>'Δημιουργήθηκε',
	'Approved'=>'Εγκρίθηκε',
	'Delivered'=>'Παραδόθηκε',
	'Cancelled'=>'Ακυρώθηκε',

	'Currency' => 'Νόμισμα',
	'LBL_BASE_CURRENCY' => 'Προεπιλεγμένο Νόμισμα',

	'Discount Percent' => 'Ποσοστό Έκπτωσης',
	'Discount Amount' => 'Ποσό Έκπτωσης',
	'Item Discount Amount' => 'Ποσό Έκπτωσης Αντικειμένου',
	'Net Price' => 'Καθαρή τιμή', 
	'Item Discount Percent' => 'Ποσοστό Έκπτωσης Αντικειμένου',

	'Due Date' => 'Προθεσμία',
	'Due Date & Time' => 'Ημερομηνία & Ώρα Προθεσμίας',
	'Email' => 'Πρωτεύον Email',
	'Secondary Email' => 'Δευτερεύον Email',
	'Other Email' => 'Άλλο Email',
	'Email Opt Out' => 'Εξαίρεση από Email',
	'Is Converted From Lead' => 'Μετατράπηκε από Δυνητικός',
	'Expected Close Date' => 'Αναμενόμενη ημερομηνία κλεισίματος',
	'Fax' => 'Φαξ',

	'Last Name' => 'Επώνυμο',
	'First Name' => 'Όνομα',

	'High' => 'Υψηλή',
	'Low' => 'Χαμηλή',
	'In Progress' => 'Σε Εξέλιξη',

	'Subject' => 'Θέμα',
	'Shipping Address'=>'Διεύθυνση Αποστολής',
	'Shipping Po Box'=>'Τ.Θ.',
	'Shipping City'=>'Πόλη Αποστολής',
	'Shipping State'=>'Νομός Αποστολής',
	'Shipping Code'=>'Τ.Κ. Αποστολής',
	'Shipping Country'=>'Χώρα Αποστολής',
	'Terms & Conditions' => 'Όροι Και Προϋποθέσεις',
	'Item Name' => 'Ονομασία Αντικειμένου',
	'Quantity' => 'Ποσότητα',
	'List Price' => 'Τιμή Πώλησης',
	'Image' => 'Εικόνα',
	'Purchase Cost' => 'Κόστος αγοράς',
	'Margin' => 'Περιθώριο',
	'Item Comment' => 'Σχολιασμός Αντικειμένου',
	'Tax1' => 'Φόρος1',
	'Tax2' => 'Φόρος2',
	'Tax3' => 'Φόρος3',
	'Excise Duty' => 'Ειδικός Φόρος Κατανάλωσης',
	'Total' => 'Σύνολο',
	'Sub Total' => 'Υποσύνολο',
	'Tax Type' => 'Φορολογικός τύπος',
	'S&H Amount' => 'Ποσόστωση ταχυδρόμησης',
	'Status' => 'Κατάσταση',
	'Vendor Name'=>'Όνομα Προμηθευτή',

	'LBL_ADDRESS_INFORMATION' => 'Λεπτομέρειες Διεύθυνσης',
	'LBL_DESCRIPTION_INFORMATION' => 'Λεπτομέρειες Περιγραφής',
	'LBL_TERMS_INFORMATION' => 'Όροι και Προϋποθέσεις',
	'LBL_PRICING_INFORMATION' => 'Πληροφορίες Τιμολόγησης',
	'LBL_RELATED_PRODUCTS' => 'Λεπτομέρειες προϊόντος',
	'LBL_REMINDER_INFORMATION' => 'Λεπτομέρειες υπενθύμισης',

	'LBL_FOLDER_SAVED' => 'Ο φάκελος αποθηκεύτηκε',
	'LBL_FOLDER_EXISTS' => 'Ο φάκελος υπάρχει ήδη',
	'LBL_FOLDER_DELETED' => 'Ο φάκελος διαγράφηκε',
	'LBL_EDIT_FOLDER' => 'Επεξεργασία φακέλου',

	'Lead Source' => 'Προέλευση Δυνητικού',
	'Mobile' => 'Κινητό Τηλέφωνο',

	'Notify Owner' => 'Ειδοποίηση κατόχου',
	'Other Phone' => 'Δευτερεύον Τηλ',
	'Phone' => 'Σταθερό Τηλέφωνο',

	'State' => 'Νομός',
	'Po Box' => 'Τ.Θ.',
	'Postal Code' => 'Τ.Κ.',

	'Potential Name' => 'Όνομα Ευκαιρίας',
	'Priority'=>'Προτεραιότητα',

	'Product Name' => 'Όνομα Προϊόντος',
	'Rating' => 'Κατάταξη',

	'Related To' => 'Σχετίζεται με',
	'Type' => 'Τύπος',

	'Reviewed'=>'Ελέγχθηκε',
	'Salutation' => 'Προσφώνηση',
	'Street' => 'Οδός',
	'Support Start Date' => 'Ημ. Έναρξης Υποστήριξης',
	'Support Expiry Date' => 'Ημ. Λήξης Υποστήριξης',
	'Sales Start Date'=>'Ημ. Έναρξης Πώλησης',
	'Sales End Date'=>'Ημ. Λήξης Πώλησης',

	//Dashboard list of widgets translation
	'Open Tickets' => 'Ανοιχτά Αιτήματα',
	'LBL_SELECT_STATUS' => 'Επιλέξτε Κατάσταση',
	'LBL_BLANK' => '-Κενό-',

	//PageTitle translations
	'LBL_VTIGER_CRM_HONEST_OPEN_SOURCE' => 'vtiger CRM On Demand',

	//Translations used in inventory modules for sopy address
	'SINGLE_Accounts' => 'Εταιρεία',
	'SINGLE_Contacts' => 'Πελάτης',

	//Weekdays in 3 letters translations
	'LBL_Sun' => 'Κυρ',
	'LBL_Mon' => 'Δευ',
	'LBL_Tue' => 'Τρι',
	'LBL_Wed' => 'Τετ',
	'LBL_Thu' => 'Πεμ',
	'LBL_Fri' => 'Παρ',
	'LBL_Sat' => 'Σαβ',

	//Months in 3 letters translations
	'LBL_Jan' => 'Ιαν',
	'LBL_Feb' => 'Φεβ',
	'LBL_Mar' => 'Μαρ',
	'LBL_Apr' => 'Απρ',
	'LBL_May' => 'Μαι',
	'LBL_Jun' => 'Ιου',
	'LBL_Jul' => 'Ιου',
	'LBL_Aug' => 'Αυγ',
	'LBL_Sep' => 'Σεπ',
	'LBL_Oct' => 'Οκτ',
	'LBL_Nov' => 'Νοε',
	'LBL_Dec' => 'Δεκ',

	'LBL_CALENDAR_SETTINGS' => 'Ρυθμίσεις Ημερολογίου',
	'Reminder Interval' => 'χρονικό διάστημα υπενθύμισης',

	//Translations used in inventory modules for address
	'SINGLE_Accounts' => 'Εταιρεία',
	'SINGLE_Contacts' => 'Πελάτης',
	'HelpDesk' => 'Αιτήματα',
	'LBL_TAG_CLOUD' => 'Ετικετοσύννεφο',

	'LBL_NO_RECORDS' => 'Καμία Εγγραφή',
	'LBL_NO_RELATED' => 'Κανένας Συσχετισμός',
	'LBL_GLOBAL_SEARCH_MAX_MESSAGE' => 'Μόνο τα 100 πρώτα αποτελέσματα των ενοτήτων εμφανίζονται παρακάτω. Για σχετικά αποτελέσματα από μια ενότητα παρακαλώ επιλέξτε την ενότητα για αναζήτηση',
	'LBL_GLOBAL_SEARCH_MAX_MESSAGE_FOR_MODULE' => 'Μόνο τα 100 πρώτα αποτελέσματα εμφανίζονται παρακάτω. Παρακαλώ χρησιμοποιήστε την Σύνθετη Αναζήτηση αν δεν ικανοποιήστε από το αποτέλεσμα',

	// used in reports detail view
	'LBL_ACTION' => 'Ενέργεια',
	'LBL_NOTE' => 'Σημείωση',
	'LBL_SELECT_OPTION' => 'Διαλέξτε μία Επιλογή',

	//Module Manager
	'LBL_TOOLTIP' => 'Διαχείριση Αναδυόμενης Βοήθειας',
	'LBL_MODULE_SEQUENCE_NUMBERING' => '%s Αρίθμησης',
	'LBL_CUSTOM_FIELD_MAPPING' => 'Προσαρμοσμένη Διασύνδεση Πεδίων',
	'LBL_WEBFORMS' => 'Δικτυακή Φόρμα',

	//Compose Email Strings
	'SINGLE_Emails' => 'Email',
	'Emails' => 'Emails',
	'LBL_SELECT_EMAIL_IDS' => 'Επιλογή Email',
	'LBL_SUBJECT' => 'Θέμα',
	'LBL_ATTACHMENT' => 'Επισυναπτόμενο',
	'LBL_BROWSE_CRM' => 'Περιήγηση CRM',
	'LBL_SEND' => 'Αποστολή',
	'LBL_SAVE_AS_DRAFT' => 'Αποθήκευση ως Πρόχειρο',
	'LBL_GO_TO_PREVIEW' => 'Προεπισκόπηση',
	'LBL_SELECT_EMAIL_TEMPLATE' => 'Επιλογή Πρότυπου Email',
	'LBL_COMPOSE_EMAIL' => 'Σύνταξη Email',
	'LBL_CC' => 'Cc',
	'LBL_BCC' => 'Bcc',
	'LBL_ADD_CC' => 'Προσθήκη Cc',
	'LBL_ADD_BCC' => 'Προσθήκη Bcc',
	'LBL_MAX_UPLOAD_SIZE' => 'Ανώτατο όριο',
	'LBL_EXCEEDED' => 'Υπερβαίνει',
	'LBL_OPERATION_NOT_PERMITTED' => 'Η ενέργεια δεν Επιτρέπεται',
	'LBL_MERGE' => 'Συγχώνευση',
	'LBL_FIND_DUPLICATES' => 'Βρείτε διπλότυπα',
	'LBL_MERGING_CRITERIA_SELECTION' => 'Επιλογή Κριτηρίων Συγχώνευσης',
	'LBL_AVAILABLE_FIELDS' => 'Διαθέσιμα Πεδία',
	'LBL_MERGE_RECORDS_DESCRIPTION' => 'Η κύρια εγγραφή θα διατηρηθεί μετά τη συγχώνευση. Μπορείτε να επιλέξετε τη στήλη για να διατηρήσουν τις τιμές.
										Η άλλη εγγραφή θα διαγραφεί, αλλά οι σχετικές πληροφορίες θα συγχωνευθούν.',
	'LBL_MERGE_RECORDS_IN' => 'Συγχώνευση εγγραφών στα',
	'LBL_FIELDS' => 'Πεδία',
	'LBL_RECORD' => 'Εγγραφές',
	'LBL_NO_DUPLICATED_FOUND' => 'Δεν βρέθηκαν διπλότυπα',
	'LBL_MERGE_SELECT' => 'Συγχώνευση Επιλεγμένων',

	//sbproducts translation
	'LBL_BACK_TO_PRODUCTS' => 'Πίσω στα Προϊόντα',

	//Translations used for Export to Pdf
	'Valid Date' => 'Έγκυρη Ημερομηνία',
	'Phone: ' => 'Σταθερό Τηλ: ',
	'Issued Date' => 'Ημερομηνία Έκδοσης',
	'Shipping & Handling Charges' => 'Χρεώσεις Ταχυδρόμησης',
	'Shipping & Handling Tax:' => 'Φόρος Ταχυδρόμησης:',
	'Discount'=>'Έκπτωση',
	'Net Total' => 'Καθαρό Σύνολο',
	'Product Code'=>'Κωδ. Προϊόντος',
	'Customer Name'=>'Όνομα Πελάτη',
	'Price'=>'Τιμή',
	'Tax:' => 'Φ.Π.Α:',
	'Contact Name' => 'Όνομα Πελάτη',
	'Grand Total:' => 'Γενικό Σύνολο:',
	'Fax: ' => 'Φαξ: ',
	'Website: ' => 'Ιστοσελίδα: ',
	'Tax' => 'Φόρος',
	'LBL_ALPHABETS' => 'Α,Β,Γ,Δ,Ε,Ζ,Η,Θ,Ι,Κ,Λ,Μ,Ν,Ξ,Ο,Π,Ρ,Σ,Τ,Υ,Φ,Χ,Ψ,Ω,',

	//Realted tab strings
	'Service Contracts' => 'Συμβόλαια Υπηρεσιών',
	'Projects' => 'Έργα',
	'Sales Order' => 'Παραγγελεία Πώλησης',
	'Purchase Order' => 'Εντολή Αγοράς',
	'Payments' => 'Πληρωμές',
	'List and Campaigns' => 'Λίστα και Καμπάνιες',
	'Project Tasks' => 'Εργασίες του Έργου',
	'Project Milestones' => 'Ορόσημα Έργου',

	'Labels Editor' => 'Επεξερ. ονομάτων Πεδίων',
	'LBL_IGNORE_EMPTY_VALUES' =>'Παράβλεψη κενών τιμών',

	//Related tabs of Products
	'Product Bundles' => 'Συνδυαστικά Πακέτα',
	'Parent Product' => 'Γονικό Προϊόν',
	'LBL_PRODUCT_BUNDLE' => 'Συνδυαστικό Πακέτο',
	'LBL_EDIT_QUANTITY' => 'Επεξεργασία Ποσότητας',
	'LBL_ADD_TO_PRODUCTS' => 'Προσθήκη Προϊόντος',

	'LBL_MANAGE_USERS' => 'Διαχείριση Χρηστών',
	'LBL_TRANSFER_OWNERSHIP' => 'Μεταβίβαση Κυριότητας',
	'LBL_SELECT_RELATED_MODULES' => 'Επιλέξτε Σχετικές ενότητες',
	'LBL_ASSIGNED_TO' => 'Ανατέθηκε σε',
	//Comments Field Labels for Workflow
	'Comment' => 'Σχόλιο',
	'Last Comment' => 'Τελευταίο Σχόλιο',
	'Last 5 Comments' => 'Τελευταία 5 Σχόλια',
	'All Comments' => 'Όλα τα Σχόλια',

	//Feedback on removing old version
	'LBL_OLD_VERSION_REMOVED_SOON' => 'Important Note on Old look',
	'LBL_FEEDBACK_ON_REMOVING_OLD_VERSION' => 'We are happy to share that old look will be available until February, 2014 . We added the following features to the new look recently<br><br>1. Pivot Reports<br>2. Custom Charts<br>3. Quick Search in Lists<br><br>If you have suggestions on new look, please share with us below.',
	'LBL_FEEDBACK_PLACEHOLDER' => 'Share your suggestions here..',
	'LBL_SUBMIT_FEEDBACK' => 'Submit feedback',
	'LBL_DONT_SHOW_AGAIN' => 'Do not show this again',
	'LBL_CONTINUE_TO_OLD_LOOK' => 'Continue to Old Look',
	'LBL_LEFT_PANEL_SHOW_HIDE' => 'Left Panel Show/Hide',

	//Products Popup View
	'NOT_A_BUNDLE' => 'Δεν είναι Συνδυαστικό πακέτο',
	'LBL_SUB_PRODUCTS' => 'Υποκείμενα-προϊόντα',
	'LBL_MARK_AS_HELD' => 'Σημείωση ως Διεξήχθη',
	'LBL_MARK_AS_COMPLETED' => 'Σημειώστε ως Ολοκληρωμένο',
	'LBL_SMS_MAX_CHARACTERS_ALLOWED' => 'Για μηνύματα κειμένου επιτρέπονται έως 160 χαρακτήρες',
	'LBL_HIDE_COMPLETED_EVENTS' => 'Απόκρυψη ολοκληρωμένων συμβάντων ημερολογίου',
	'LBL_SETUP_WEBFORMS' => 'Ρυθμίσεις Δικτυακής Φόρμας',
	'LBL_SPECIAL_OPTIONS' => 'Ειδικές επιλογές',
	'LBL_PARENT_OWNER' => 'Κάτοχος γονικής Εγγραφής',
	'LBL_PASSWORD_LINK_EXPIRED_OR_INVALID_PASSWORD' => 'Ο σύνδεσμος πρόσβασης έχει λήξει ή έχετε πληκτρολογήσει μη έγκυρο συνθηματικό πρόσβασης',

	// Recurring Invoice
	'Half-Yearly' => 'Εξαμηνιαία',
	'LBL_FULLSCREEN' => 'Προβολή πλήρους οθόνης',
	'LBL_SETUP_WEBFORMS' => 'Ρυθμίσεις Δικτυακής Φόρμας',
	'LBL_MODULE_DISABLED' => ' Η ενότητα είναι απενεργοποιημένη',
	'Organization Name' => 'Όνομα Εταιρείας',
	'LBL_SOCIAL_TWITTER_WIDGET' => 'Δραστηριότητα Twitter',
	'LBL_WEEK' => 'Εβδομάδα',
	//Convert Lead and Potential
	'CANNOT_CONVERT' => 'Δεν είναι δυνατή η μετατροπή',
	'LBL_CONVERT_ERROR_TITLE' => 'Οι ενότητες είναι απενεργοποιημένες',
	'LBL_FOLLOWING_ARE_POSSIBLE_REASONS' => 'Πιθανές αιτίες περιλαμβάνουν:',
	'LBL_MANDATORY_FIELDS_ARE_EMPTY' => 'Τα υποχρεωτικά πεδία είναι κενά',

	// SMSNotifier Phone Format Warning
	'LBL_PHONE_FORMAT_WARNING' => 'Βεβαιωθείτε ότι ο αριθμός τηλεφώνου ακολουθεί την διεθνή μορφή E.164',
	'LBL_PHONEFORMAT_WARNING_CONTENT' => 'Η μορφοποίηση αριθμού E.164 μπορεί να έχει δεκαπέντε ψηφία και συνήθως γράφεται ως εξής: [+] [κωδικός χώρας] [αριθμός συνδρομητή, συμπεριλαμβανομένου του κωδικού περιοχής]
										 Για παράδειγμα, για να μετατρέψετε τον αριθμό τηλεφώνου των ΗΠΑ (415 599 2671) σε μορφή E.164, θα χρειαστεί να προσθέσετε το πρόθεμα \'+\' και τον κωδικό χώρας (που είναι 1) μπροστά από τον αριθμό (+1 415 599 2671). Στο Ηνωμένο Βασίλειο και σε πολλές άλλες χώρες διεθνώς, η τοπική κλήση απαιτεί την προσθήκη ενός αριθμού 0 μπροστά από τον αριθμό συνδρομητή. Ωστόσο, για να χρησιμοποιηθεί η μορφοποίηση του E.164, αυτό το 0 πρέπει να αφαιρεθεί. Ένας αριθμός όπως το 020 7183 8750 στο Ηνωμένο Βασίλειο θα διαμορφωθεί ως +44 20 7183 8750.',
	'LBL_WARNING' => 'Προειδοποίηση',
	'LBL_ORIGIN' => 'Προέλευση',
	'SENT' => 'Απεσταλμένα',
	'RECEIVED' => 'Ληφθέντα',
	'QUEUED'=> 'Σε Αναμονή',
	'FAILED'=> 'Απέτυχαν',
	'SAVED' => 'Πρόχειρα',
	'MailManager' => 'Διαχείριση Email',
	'MAILSCANNER' => 'Μετατροπή Email',
	'LBL_SENDER_NAME' => 'Όνομα Αποστολέα',
	'SMSNotifier'=>'Ειδοποίηση με SMS',

	'Mobile Call' => 'Κλήση Κινητού',
	'LBL_NO_MORE_RESULTS' => 'Δεν υπάρχουν περισσότερα αποτελέσματα!',
	'Related To' => 'Σχετίζονται με',

	// Date Conditions
	'LBL_LESS_THAN_DAYS_AGO' => 'Λιγότερο από ημέρες πριν',
	'LBL_MORE_THAN_DAYS_AGO' => 'Πριν από αρκετές ημέρες',
	'LBL_IN_LESS_THAN' => 'Σε λιγότερο από',
	'LBL_IN_MORE_THAN' => 'Σε περισσότερες από',
	'LBL_DAYS_AGO' => 'Μέρες πριν',
	'LBL_DAYS_LATER' => 'Μέρες μετά',
	'LBL_LESS_THAN_HOURS_BEFORE' => 'Λιγότερο από ώρες πριν',
	'LBL_LESS_THAN_HOURS_LATER' => 'Λιγότερο από ώρες αργότερα',
	'LBL_MORE_THAN_HOURS_BEFORE' => 'Πάνω από ώρες πριν',
	'LBL_MORE_THAN_HOURS_LATER' => 'Περισσότερες από τις ώρες αργότερα',
	'LBL_INTERNAL_COMMENT' => 'Εσωτερικό σχόλιο',
	'LBL_NOTE_EXISTING_ATTACHMENTS_WILL_BE_REPLACED' => 'Σημείωση: Τα υπάρχοντα συνημμένα (εικόνες / αρχεία) θα αντικατασταθούν',

	//common standard fields
	'Created By' => 'Δημιουργήθηκε από',
	'LBL_SEND_PDF_FOR_SIGNING' => 'Αποστολή PDF για υπογραφή',
	'LBL_CLICK_TO_EDIT' => 'Κάντε κλικ για επεξεργασία',
	'LBL_SIGNATURE_BLOCK' => 'Υπογραφή',
	'LBL_INCLUDE_SIGNATURE_INFO' => 'Η υπογραφή που έχει οριστεί στις Προτιμήσεις μου θα επισυναφθεί στο κάτω μέρος του email',
	'LBL_INCLUDE_SIGNATURE' => 'Συμπεριλάβετε την υπογραφή',

	// module builder created module first block label 
	'LBL_BASIC_INFORMATION' => 'Βασικές πληροφορίες',
	'LBL_CLICK_HERE'=>'Κάντε κλικ εδώ',

	//Recipient preferences labels
	'LBL_SELCT_EMAIL_RECIPIENTS' => 'Επιλέξτε παραλήπτες του email',
	'LBL_REMEMBER_MY_PREF' => 'Απομνημόνευση της προτίμησής μου',
	'LBL_RECIPIENT_PREFS'=>'Προτιμήσεις παραλήπτη',
	'LBL_EMAIL_RECIPIENT_PREFS'=>'Προτιμήσεις παραλήπτη email',
	'LBL_ERROR_SAVING_PREF' => 'Σφάλμα κατά την αποθήκευση προτιμήσεων. Παρακαλώ δοκιμάστε αργότερα!',
	'LBL_NO_PREF_GIVEN' => 'Δεν δόθηκε καμία προτίμηση!',
	'LBL_PLEASE_ADD_EMAIL_FIELDS' => 'Προσθέστε πεδία email και δοκιμάστε ξανά',
	'LBL_RECIPIENT_SAVE_MESSAGE'=>'Αποθήκευση Επιτυχής. Αυτές οι προτιμήσεις θα αντιμετωπίζονται ως email προορισμού όταν στέλνετε email από αυτήν την ενότητα',
	'LBL_PREF_RESET_MESSAGE' => 'Οι προτιμήσεις email παραλήπτη έχουν επαναφερθεί!',

	'LBL_COMPANY_DETAILS' => 'Στοιχεία Εταιρείας',
	'COMPANY_LOGO_HELP_TEXT' => 'Μεταφορτώστε το όμορφο λογότυπο της εταιρείας σας. <br>Εκτός από την εμφάνιση του λογότυπου της εταιρείας σας στο CRM σας,<br>
									το όνομα της εταιρείας σας και τα στοιχεία διευθύνσεών σας χρησιμοποιούνται στις καμπάνιες μέσω <br>Email (απαιτούνται για την τήρηση των κανονισμών)<br>
									και σε προσφορές.',
	'COMPANY_LOGO_HELP_TEXT_2' =>"Μπορείτε να επεξεργαστείτε τα στοιχεία της εταιρείας αργότερα από τις <br>
									'Ρυθμίσεις CRM > Πρότυπα > Λεπτομέρειες Εταιρείας'",
	'LBL_CHOOSE_PREFERENCES' => 'Επιλέξτε τις Προτιμήσεις σας',
	'PREFERENCES_HELP_TEXT' => 'Βασικό Συνάλλαγμα - Επιλέξτε το πρωτεύον συνάλλαγμα σας. Εάν χρησιμοποιείτε πολλαπλά<br> 
								συναλλάγματα, όλες οι τιμές μετατρέπονται στο βασικό συνάλλαγμα και αποθηκεύονται,<br>
								χρησιμοποιώντας την συναλλαγματική διαφορά που διαμορφώνεται στο CRM.',
	'PREFERENCES_HELP_TEXT_2' => 'Οι προσωπικές προτιμήσεις μπορούν να ρυθμιστούν από κάθε χρήστη στην ομάδα σας.<br>
									Αυτές περιλαμβάνουν τη ζώνη ώρας, τη γλώσσα, τη μορφή ημερομηνίας, τη μορφή αριθμών.<br>
									Οι μορφές αριθμών χρησιμοποιούνται για συναλλάγματα και άλλα <br>
									πεδία αριθμό στο CRM<br>
									Μπορείτε να επεξεργαστείτε τις προτιμήσεις σας αργότερα από \'Όνομα Χρήστη > Οι προτιμήσεις μου \'',
	'LBL_CANNOT_BE_CHANGED_LATER' => 'Δεν μπορεί να αλλάξει αργότερα',
	'IMPORT_CONTACTS_GOOGLE_HELP_TEXT' => 'Μπορούμε να εισάγουμε επαφές από την Google και <br> μπορείτε επίσης να ενεργοποιήσετε τον συγχρονισμό για να διατηρήσετε τις<br>
									επαφές Vtiger και Google (στην επιλεγμένη ομάδα) ενημερωμένες.<br>
									Κάντε κλικ στο κουμπί για να εξουσιοδοτήσετε και να λάβετε τις Επαφές από την Google.',
	'IMPORT_CONTACTS_CSV_HELP_TEXT' => 'Μπορείτε να εισαγάγετε επαφές από αρχείο CSV.',
	'IMPORT_CONTACTS_CSV_HELP_TEXT2' => 'Για να εισαγάγετε Ευκαιρίες, Δυνητικούς, Αιτήματα, αναζητήστε για Ενέργειες > κουμπί Εισαγωγής σε οποιοδήποτε από τις
										 προβολές λίστας.',
	'LBL_DOEST_FIRST_ROW_HEADER' => 'Η πρώτη γραμμή στο αρχείο σας περιέχει κεφαλίδες στηλών;',
	'LBL_SELECT_CSV_FILE' => 'Επιλέξτε αρχείο CSV',
	'LBL_START_IMPORTING' => 'Ξεκινήστε την εισαγωγή',
	'LBL_PRIMARY' => 'Πρωτεύον',
	'LBL_GOOGLE_SYNC_INTIATED_MSG' => 'Η εισαγωγή σας έχει προγραμματιστεί, οι επαφές σας θα εισαχθούν σύντομα. <br> Η μετάβαση στο Επόμενο βήμα δεν θα διακόψει την εισαγωγή.',
	'LBL_IMPORT_SYNC_INTIATED_MSG' => 'Η εισαγωγή σας βρίσκεται σε εξέλιξη. Θα λάβετε ένα μήνυμα ηλεκτρονικού ταχυδρομείου όταν ολοκληρωθεί η εισαγωγή. <br> Η μετάβαση στο Επόμενο βήμα δεν θα διακόψει την εισαγωγή.',
	'LBL_TELL_US_ABOUT_YOU' => 'Βοηθήστε μας ενημερώνοντάς μας για την Εταιρεία σας',
	'TELL_US_ABOUT_YOU_HELP_TEXT' => 'Συνεχίζουμε να βελτιώνουμε το CRM και <br>
									 γνωρίζοντας καλύτερα τους χρήστες μας θα μας βοηθήσει να δώσουμε προτεραιότητα στις βελτιώσεις.',
	'TELL_US_ABOUT_YOU_HELP_TEXT_2' => 'Tτα δεδομένα που υποβάλατε δεν θα αποκαλυφθούν σε κανέναν <br>
										έξω από την Vtiger.',
	'LBL_INVALID_IMAGE' => 'Μη έγκυρη εικόνα',
	'LBL_IMPORT_CONTACTS' => 'Εισαγωγή επαφών',
	'LBL_FROM_GOOGLE' => 'Απο Google',
	'LBL_FROM_CSV' => 'Απο αρχείο CSV',
	'LBL_NEXT' => 'Επόμενο',
	'LBL_FINISH' => 'Τέλος',
	'LBL_GET_CONTACTS_FROM_GOOGLE' => 'Λήψη επαφών από την Google',
	'LBL_PHONE_NUMBER' => 'Αριθμός Τηλεφώνου',
	'LBL_NUMBER_OF_EMPLOYEES' => 'Αριθμός εργαζομένων',
	'LBL_INDUSTRY' => 'Βιομηχανία',
	'LBL_HELP_DESK_EMAIL_HELP_TEXT' => 'Οι ειδοποιήσεις σχετικά με το αίτημα θα αποστέλλονται από αυτήν τη διεύθυνση email',
	'LBL_SOCIAL_DETAILS' => 'Σύνδεση με τα κοινωνικά δίκτυα για την εξεύρεση νέων πελατών και την ανταπόκριση σε ερωτήματα',
	'LBL_SOCIAL_AUTHORIZE_HELP'=>'Συνδεθείτε με τον λογαριασμό twitter της εταιρείας σας για να δημοσιεύσετε τα μηνύματά σας και να παρακολουθήσετε την αφοσίωση',
	'LBL_SOCIAL_AUTHORIZE' => 'Συνδεθείτε με το Twitter',
	'LBL_SOCIAL_LINKED' => 'Το Vtiger σας είναι τώρα συνδεδεμένο με',
	'LBL_ENTER_KEYWORDS_HELP'=>'Το Vtiger θα σαρώσει το Twitter και θα βρει μηνύματα με αυτές τις λέξεις',
	'LBL_ENTER_KEYWORDS' => 'Εισάγετε λέξεις - κλειδιά',

	 // Social Module Strings
	'LBL_TWEETS' => 'Tweets',
	'LBL_FAVOURITES' => 'Αγαπημένα',
	'LBL_FOLLOWING' => 'Ακολουθείτε',
	'LBL_FOLLOWERS' => 'Ακόλουθοι',
	'LBL_VIEW_FULL_PROFILE' => 'Προβολή πλήρους προφίλ',
	'LBL_LOCATION' => 'Τοποθεσία',
	'LBL_DESCRIPTION' => 'Περιγραφή',
	'LBL_JOINED' => 'Εντάχθηκε',
	'LBL_SEARCH_PROFILES_IN_TWITTER' => 'Αναζήτηση των προφίλ στο Twitter',
	'LBL_CHOOSE_ANOTHER_PROFILE' => 'Επιλέξτε Άλλο προφίλ',
	'LBL_ADD_TICKET' => 'Προσθήκη αιτήματος',
	'LBL_ADD_OPPORTUNITY' => 'Προσθήκη Ευκαιρίας',
	'LBL_RETWEET' => 'Retweet',
	'LBL_RETWEETS' => 'Retweets',
	'LBL_UNDO_RETWEET' => 'Αναίρεση Retweet',
	'LBL_FAVOURITE' => 'Αγαπημένο',
	'LBL_UNDO_FAVOURITE' => 'Αναίρεση Αγαπημένου',
	'LBL_TWITTER_HANDLER_MSG' => 'Για να δείτε τη σύνοψη και το ιστορικό του προφίλ της επαφής, προσθέστε το όνομα χρήστη του Twitter ή τα προφίλ αναζήτησης χρησιμοποιώντας τον παρακάτω σύνδεσμο.',
	'LBL_ADD_EVENT_OR_TODO' => 'Προσθήκη Συμβάντος/Εργασίας',
	'LBL_ADD_TICKET_CONTACT' => 'Προσθέστε Αιτήματος + Επαφής',
	'LBL_ADD_CASE_CONTACT' => 'Προσθέστε Υπόθεση + Επαφή',
	'LBL_ADD_OPPORTUNITY_CONTACT' => 'Προσθήκη Ευκαιρίας + Επαφή',
	'LBL_TWITTER_HANDLER_DETAILS' => 'Στοιχεία χρήστη Twitter',
	'LBL_PRIMARY_TWITTER' => 'Όνομα χρήστη Twitter',
	'LBL_ADD_TWITTER_HANDLER' => 'Add Twitter Username',
	'LBL_VIEW_TICKET' => 'Προβολη Αιτήματρος',
	'LBL_VIEW_CASE' => 'Προβολή Υπόθεσης',
	'LBL_VIEW_OPPORTUNITY' => 'Προβολή Ευκαιρίας',
	'LBL_CLICK_TO_SELECT_PROFILE' => 'Κάντε κλικ για να επιλέξετε προφίλ',
	'LBL_INVALID_IMAGE' => 'Μη έγκυρη εικόνα',
	'LBL_NEXT' => 'Επόμενο',
	'LBL_FINISH' => 'Τέλος',
	// Module field to store Source of Record
	'Source' => 'Πηγή',

	'LBL_DEFAULT_REPLY_TO' => 'Προεπιλεγμένη Απάντηση σε emails', 
	'outgoing_server_from_email' => 'Διακομιστής εξόδου για τα πεδία : Από email', 
	'hepldesk_support_email' => 'Email Τμήματος Υποστήριξης', 
	'user_primary_email' => 'Πρωτεύων email χρήστη', 
	'LBL_DEFAULT_REPLY_TO_INFO' => 'Αυτή η ρύθμιση ισχύει μόνο για τα απευθείας email μέσω του CRM. Δεν επηρεάζει τα μηνύματα ηλεκτρονικού ταχυδρομείου που αποστέλλονται από ροές διεργασιών, καμπάνιες με email κλπ.', 
	'search_in_all_fields'=>'Αναζήτηση σε Όλα τα πεδία',
	'search_in_name_field'=>'Αναζήτηση στο πεδίο Όνομα',
	'LBL_LOOKUP_SEARCH_MODE_INFO'=>'Παράδειγμα: Στη φόρμα δημιουργίας επαφών, όταν εισάγετε κείμενο στο πλαίσιο ονομάτων εταιρείας, αυτή η ρύθμιση θα ελέγχει ποια πεδία στην Εταιρεία θα μπορούν να αναζητηθούν',

	'VAT' => 'ΦΠΑ',
	'Sales' => 'Πωλήσεις',
	'Service' => 'Υπηρεσία',
	'Pre Tax Total' => 'Προ φόρων Σύνολο',
	'Received' => 'Ελήφθη',
	'Balance' => 'Υπόλοιπο',
	'S&H Amount' => 'Χρεώσεις',
	'S&H Percent' => 'Φόροι επί των χρεώσεων',
	'Tax Region' => 'Εύρος φόρου',

	//Singular Modulenames
	'SINGLE_Potentials' => 'Ευκαιρία',
	'SINGLE_HelpDesk' => 'Δελτίο υποστήριξης',
	'SINGLE_Accounts' => 'Εταιρεία',
	'SINGLE_Contacts' => 'Πελάτης',
	'SINGLE_Project' => 'Έργο',
	'LBL_ROLLUP_COMMENTS_INFO' => "Αν η επιλογή 'Συγκεντρωτικά σχόλια' έχει οριστεί σε 'Ενεργή', θα εμφανιστούν τα σχόλια για τις σχετικές εγγραφές. 
								 Για παράδειγμα, αν βλέπετε σχόλια σε μια εγγραφή εταιρείας, θα εμφανιστούν και σχόλια που έχουν προστεθεί στους σχετικούς Επαφές, Ευκαιρίες, Αιτήματα, ..κλπ.",
	'LBL_ROLLUP_COMMENTS' => "Συγκεντρωτικά σχόλια",

	//others
	'Engagement Score' => 'Βαθμολογία Αφοσίωσης',
	'group' => 'Ομάδα',
	'individual' => 'Άτομο',
	'LBL_VIEW_FILE'=>'Προβολή αρχείου',
	'LBL_DOWNLOAD_FILE' => 'Λήψη αρχείου',
	'LBL_PREVIEW_NOT_AVAILABLE' => 'Η προεπισκόπηση δεν είναι διαθέσιμη',
	'LBL_PREVIEW_SUPPORTED_FILES' => '<b><strong>Υποστηριζόμενοι τύποι αρχείων : </strong></b><br><br><b>Αρχεία Pdf</b><br><b>Αρχεία κειμένου - </b>txt,csv,ics<br><b>Αρχεία Open Document - </b>open document κείμενο(odt),open document λογιστικό φύλλο(ods) και open document παρουσίαση(odp)<br><b>Αρχεία πολυμέσων - </b>αρχεία εικόνας, ήχου & βίντεο<br>',
	'Mailing Address' => 'Ταχυδρομική διεύθυνση',
	'Other Address' => 'Άλλη διεύθυνση',
	'LBL_PREVIOUS' => 'Προηγούμενο',
	'LBL_NEXT' => 'Επόμενο',
	'Workflows' => 'Ροή διεργασιών',
	'Primary Twitter' => 'Όνομα χρήστη Twitter',

	/* Vtiger7 Language Strings */
	'LBL_CLICK_HERE_TO_SELECT_ALL_RECORDS'=>'Επιλέξτε όλες τις εγγραφές σε αυτήν τη σελίδα',
	'LBL_CLICK_HERE_TO_MANAGE_LIST_COLUMNS'=>'Κάντε κλικ εδώ για να διαχειριστείτε τις στήλες λίστας',
	'LBL_SHOW_MORE' =>'Δείτε περισσότερα',
	'LBL_DISPLAYING_RESULTS' => 'Εμφάνιση αποτελεσμάτων',
	'LBL_ADD_STAR' => 'Προσθήκη Αστεριού',
	'LBL_REMOVE_STAR' => 'Κατάργηση Αστεριού',
	'LBL_UPDATE_LIST' => 'Λίστα ενημερώσεων',
	'LBL_SHARE_THIS_LIST' => 'Μοιραστείτε αυτήν τη λίστα',
	'LBL_ADD_USERS_ROLES' => 'Προσθέστε χρήστες και ρόλους',
	'LBL_LISTS' => 'Λίστες',
	'LBL_SEARCH_FOR_LIST' => 'Αναζήτηση για λίστα',
	'LBL_MY_LIST' => 'Η λίστα μου',
	'LBL_SHARED_LIST' => 'Λίστα κοινής χρήσης',
	'LBL_MERGE_SELECTED_RECORDS' => 'Συγχώνευση επιλεγμένων εγγραφών',
	'LBL_MATCH_CRITERIA' => 'Κριτήριο Διπλότυπο αναζήτησης',
	'LBL_MATCH_FIELDS' => 'Ταίριασμα πεδίων',
	'LBL_EDIT_CUSTOM'=>'Επεξεργασία λίστας',
	'LBL_SHOW_MAP'=>'Προβολή στον Χάρτη',
	'LBL_ADD_TAB'=>'Προσθήκη καρτέλας',
	'LBL_TAB_NAME'=>'Όνομα Πίνακα ελέγχου',
	'LBL_SHARED_TAGS_ACCESS'=>'Οι κοινές ετικέτες είναι προσβάσιμες από όλους τους χρήστες του Vtiger',
	'LBL_GOTO_TAGS'=>'Μεταβείτε στις Ρυθμίσεις>Οι προτιμήσεις μου>Οι ετικέτες μου, για Επεξεργασία ή Διαγραφή ιδιωτικών ετικετών',
	'LBL_UNLINK'=>'Κατάργηση Συνδέσμου',
	'LBL_SWITCH_TO_OLD'=>'Μεταβείτε στην παλιά έκδοση',
	'LBL_SLA_INFORMATION' => 'Πληροφορίες συμφωνίας σε επίπεδο υπηρεσιών (SLA)',

	//configure columns
	'LBL_UPDATE_LIST' => 'Λίστα ενημερώσεων',
	'LBL_CONFIG_COLUMNS' => 'Ρυθμίστε τις στήλες',
	'LBL_SELECTED_FIELDS' => 'Επιλεγμένα πεδία',
	'LBL_AVAILABLE_FIELDS' => 'Διαθέσιμα πεδία',
	'LBL_SEARCH_FIELDS' => 'Πεδία αναζήτησης',

	// Extensions
	'ExtensionStore' =>'Κατάστημα Επεκτάσεων',
	'LBL_EXTENSIONS' => 'Επεκτάσεις',
	'LBL_SYNC_LOG' => 'Log συγχρονισμού',
	'LBL_SYNC_SETTINGS' => 'Ρυθμίσεις συγχρονισμού',
	'LBL_SYNC_NOW' => 'Συγχρονισμός τώρα',
	'LBL_DATE' => 'Ημερομηνία',
	'LBL_TIME' => 'Ώρα',
	'LBL_SKIPPED' => 'Παραλείφθηκε',
	'vt_create' => 'Εγγραφές που δημιουργήθηκαν στο Vtiger',
	'vt_update' => 'Εγγραφές ενημερώθηκαν στο Vtiger',
	'vt_delete' => 'Εγγραφές διαγράφηκαν στο Vtigerr',
	'vt_skip' => 'Οι εγγραφές παραλείφθηκαν στο Vtiger',
	'app_create' => 'Εγγραφές που δημιουργήθηκαν στην %s',
	'app_update' => 'Εγγραφές που ενημερώθηκαν στην %s',
	'app_delete' => 'Εγγραφές που διαγράφηκαν στην %s',
	'app_skip' => 'Εγγραφές που παραλείφθηκαν στην %s',
	'LBL_DOWNLOAD_AS_CSV' => 'Λήψη ως CSV',
	'LBL_SOURCE_MODULE' => 'Πηγή Ενότητας',
	'LBL_RECORD_NAME' => 'Όνομα εγγραφής',
	'LBL_REASON' => 'Αιτιολόγηση',
	'LBL_SELECT_MODULES_TO_SYNC' => 'Επιλέξτε ενότητες για συγχρονισμό',
	'LBL_DATA' => 'Δεδομένα',
	'LBL_ENABLE_SYNC' => 'Ενεργοποίηση συγχρονισμού',
	'LBL_FIELD_MAPPING' => 'Διασύνδεση πεδίων',
	'LBL_SYNC_DIRECTION' => 'Κατεύθυνση συγχρονισμού',
	'LBL_SYNC_BOTH_WAYS' => 'Συγχρονίστε αμφίδρομα',
	'LBL_SAVE_SETTINGS' => 'Αποθηκεύσετε τις ρυθμίσεις',
	'LBL_CONFIGURE' => 'Διαμορφώστε',
	'LBL_VIEW' => 'Προβολή',
	'LBL_CHANGE_USER' => 'Αλλαγή χρήστη',

	'LBL_CREATE_LIST' => 'Δημιουργία νέας λίστας',
	'sent' => 'Απεσταλμένα',
	'accepted' => 'Αποδεκτό',
	'SINGLE_Users' => 'Χρήστης',
	'LBL_RECENT_COMMENTS' => 'Πρόσφατα σχόλια',
	'LBL_ENGAGEMENT_HISTORY' => 'Ιστορικό αφοσίωσης',
	'LBL_NO_ENGAGEMENTS_FOUND' => 'Δεν υπάρχουν στοιχεία αφοσίωσης',
	'LBL_MARKETING' => 'MARKETING',
	'LBL_SALES' => 'ΠΩΛΗΣΕΙΣ',
	'LBL_PROJECT' => 'ΕΡΓΑ',
	'LBL_TOOLS' => 'ΕΡΓΑΛΕΙΑ',
	'LBL_TAGS' => 'Ετικέτες',
	'LBL_SHARE_TAGS' => 'Δημόσιες ετικέτες',
	'LBL_SHARE_TAG' => 'Κοινή ετικέτα(ες)',
	'private' => 'ιδιωτικό',
	'Tags' => 'Ετικέτες',
	'public' => 'κοινόχρηστο',
	'Private/Shared' => 'Ιδιωτικό/Κοινόχρηστο',
	'LBL_TAG_FOR' => 'Ετικέτες για %s',
	'LBL_CURRENT_TAGS' => 'Τρέχουσες ετικέτες',
	'LBL_CREATE_NEW_TAG'=> 'Δημιουργία νέας ετικέτας',
	'LBL_ADD_TAG' => 'Προσθήκη ετικέτας',
	'LBL_REMOVE_TAG' => 'Κατάργηση ετικέτας',
	'LBL_TAG_SEPARATOR_DESC' => 'Χρησιμοποιήστε κόμμα για να διαχωρίσετε πολλές ετικέτες',
	'LBL_ENTER_TAG_NAME'=> 'Εισάγετε το όνομα της ετικέτας',

	'LBL_SELECT_COLOR' => 'Επιλέξτε Χρώμα',
	'LBL_NO_TAG_EXISTS' => 'Δεν υπάρχει ετικέτα',
	'LBL_ADD_NEW_TAG' => 'Προσθήκη νέας ετικέτας',
	'LBL_SELECT_EXISTING_TAG' => 'Πληκτρολογήστε εδώ για να επιλέξετε μια υπάρχουσα ετικέτα',
	'LBL_CREATE_NEW_TAG' => 'Δημιουργία νέας ετικέτας',
	'LBL_ADD_OR_SELECT_TAG' => 'Προσθήκη/Επιλογή ετικέτας',
	'LBL_SELECT_FROM_AVAIL_TAG' => 'Επιλέξτε από τις διαθέσιμες ετικέτες',
	'LBL_MARKET_PLACE' => 'Market Place',

	'LBL_SHARE_THIS_LIST' => 'Μοιραστείτε τη λίστα',
	'LBL_ADD_USERS_ROLES' => 'Προσθήκη Χρηστών, Ρόλων...',
	'EmailTemplates' => 'Πρότυπα Email',
	'LBL_BILLING' => 'Χρέωση',
	'LBL_ENABLED' => 'Ενεργοποιήθηκε',

	//Title added for buttons
	'LBL_CREATE_FILTER' => 'Δημιουργία φίλτρου',
	'LBL_IS' => 'είναι',
	'LBL_CREATION' => 'δημιουργία',
	'LBL_UPLOAD' => 'Μεταφόρτωση',
	'LBL_ATTACH_FILES' => 'Επισύναψη αρχείων',
	'LBL_KEY_FIELDS' => 'Βασικά πεδία',
	'LBL_ADD_TODO' => 'Προσθήκη εργασίας',

	"LBL_VIEW_DETAILS" => "Δείτε λεπτομέρειες",

	//Engagements History
	"LBL_HAPPENED_ON" => "Συνέβη στις",
	"LBL_HAPPENED" => 'Συνέβη',
	"LBL_TYPE" => "Τύπος",
	"LBL_STATUS" => "Κατάσταση",
	"LBL_CUSTOMER" => "Σχετίζονται με",
	"LBL_SUMMARY" => "Σύνοψη",
	"LBL_ENGAGEMENTS" => "Στοιχ. Αφοσίωσης",

	'LBL_PLEASE_SELECT_ATLEAST_ONE_OPTION' => 'Διαλέξτε τουλάχιστον μία επιλογή',
	'LBL_STARRED' => 'Κάντε κλικ για να καταργήσετε την παρακολούθηση',
	'LBL_UNSTARRED' => 'Παρακολούθηση',
	'LBL_EDIT_EMAIL_PREFERENCE_TOOLTIP' => 'Επεξεργαστείτε αυτές τις προτιμήσεις από το αναπτυσσόμενο μενού Ρυθμίσεις στις λεπτομέρειες τωn ενοτήτων ή στην προβολή λίστας',
	'LBL_SYSTEM' =>'Σύστημα',
	'LBL_SHOW_MAP' => 'Εμφάνιση Χάρτη',
	'LBL_GOOGLE_MAP' => 'Google Map',
	'LBL_CREATION' => 'δημιουργία',
	'LBL_ADD_TODO' => 'Προσθήκη εργασίας',

	'LBL_KEY_METRICS' => 'Βασικές μετρήσεις',

	//Print template strings
	'LBL_TEMPLATE_NAME' => 'Όνομα προτύπου',
	'LBL_MODULE' => 'Ενότητα',
	'LBL_IS_DEFAULT' => 'Προεπιλεγμένο πρότυπο',
	'LBL_SELECT_RECORD_FIELD' => 'Επιλέξτε πεδίο εγγραφής',
	'LBL_PREVIEW' => 'Προεπισκόπηση',
	'LBL_UNLINKED' => 'Αποσυνδεδεμένο',

	'LBL_FOLLOW' => 'Παρακολούθηση',
	'LBL_UNFOLLOW' => 'Κατάργηση παρακολούθησης',
	'Any' => 'Οτιδήποτε',
	'LBL_NA' => '-NA-',
	'LBL_DISABLED' => 'Απενεργοποίηση',

	//help
	'LBL_HELP_ANSWERS' => 'Απαντήσεις',
	'LBL_HELP_ARTICLES' => 'Άρθρα',
	'LBL_HELP_TOUR' => 'Περιήγηση',
	'LBL_NO_HELP_ARTICLE_FOUND' => 'Λυπούμαστε, δεν βρέθηκε κανένα άρθρο',
	'LBL_HELP_SEARCH' => 'Αναζήτησε βοήθεια',
	'LBL_HELP_BROWSE_ALL' => 'Περιήγηση σε όλα',

	'LBL_EXPORT_FILE_NAME' => 'Όνομα αρχείου Εξαγωγής',
	'LBL_EXTENSION_NOT_ALLOWED_FOR_NONADMIN' => '%s Ο συγχρονισμός επιτρέπεται μόνο για διαχειριστές',
	'SINGLE_Invoice' => 'Τιμολόγιο',
	'SINGLE_SalesOrder' => 'Παραγγελία πώλησης',
	'SINGLE_PurchaseOrder' => 'Εντολή αγοράς',
	'SINGLE_Quotes' => 'Προσφορά',
	'LBL_POST_YOUR_COMMENT_HERE' => 'Δημοσιεύστε το σχόλιό σας εδώ',
	// Phone Calls
	'LBL_CLICK_TO_CALL' => 'Κλικ-για-Κλήση',
	'LBL_SHOW_TOTAL_NUMBER_OF_RECORDS' => 'Κάντε κλικ για το μέγεθος λίστας',

	'LBL_GENERAL_FIELDS' => 'Γενικά πεδία',
	'LBL_ROLL_UP' => 'Σύμπτυξη',

	'LBL_TOUCH_POINTS' => 'Σημεία επαφής',
	'LBL_HISTORY' => 'Ιστορικό',
	'LBL_ENGAGEMENT_HISTORY' => 'Ιστορικό σημείων επαφής',
	'LBL_SETTINGS' => 'Ρυθμίσεις',

	//List View and thumbnail view button labels
	'LBL_LIST_VIEW'=>'Προβολή Λίστας',
	'LBL_THUMBNAIL_VIEW'=>'Προβολή μικρογραφιών',
	'LBL_CHART_VIEW'=>'Προβολή Διαγράμματος',
	'LBL_EMAIL_SUPPORT' => 'Email Υποστήριξης',
	'LBL_NO_ENTITIES_FOUND' => 'Κανένα %s δεν βρέθηκε',

	'LBL_TRANSFER_ALL_CASES_TO' => 'Μεταφορά όλων των υποθέσεων σε',
	'LBL_SLA_TO_BE_DELETED' => 'SLA To Be Deleted',
	'LBL_TO_USE_SWITCH_TO_NEW_LOOK' => 'To use <b>%s</b>, we recommend you to Switch to new look.',
	'LBL_INTERNAL_COMMENT_INFO' => 'Το σχόλιο θα προβληθεί από τους χρήστες του CRM, μόνο εάν είναι ενεργοποιημένο το πλαίσιο ελέγχου εσωτερικού σχολιασμού. Για να ενημερώσετε τους πελάτες (μέσω διαμορφωμένων ροών διεργασίας ή πύλης πελατών) αφήστε αυτό το πλαίσιο ελέγχου απενεργοποιημένο',
	'LBL_CHANGE' => 'Αλλαγή',
	'LBL_REVOKE_ACCESS' => 'Αναίρεση πρόσβασης',
	'GOOGLE_REVOKE_ACCESS_MESSAGE' => 'Clicking on this will remove existing Google authentication for Contacts and Calendar. It will not delete any records which are already synchronized.',
	'OFFICE_REVOKE_ACCESS_MESSAGE' => 'Clicking on this will remove existing Office365 authentication for Contacts, Calendar and Emails. It will not delete any records which are already synchronized.',
	'LBL_START_GUIDED_TOUR' => 'Start Guided Tour',
	'LBL_MENU' => 'Μενού',
	'LBL_EMAIL_DOMAIN_HELPTEXT'=>'Email Domain field will be used to match Cases and Contacts created from Emails. '
	. 'For example, if a new support email comes from john@company.com, it would be linked to Organization that has '
	. 'company.com in the Email domain field. Multiple domains should be separated by commas',

	'LBL_SHARED_LIST_NON_ADMIN_MESSAGE' => 'Αυτή είναι μια κοινόχρηστη λίστα. Επικοινωνήστε με τον διαχειριστή του CRM για να ρυθμίσετε τις στήλες για το: Όλες οι λίστες',
	'LBL_SHARED_LIST_OWNER_MESSAGE' => 'Αυτή είναι μια κοινόχρηστη λίστα. Επικοινωνήστε με τον %s για να αλλάξει τις στήλες αυτής της λίστας',

	'Mr.' => 'Κύρ.',
	'Ms.' => 'Δς.',
	'Mrs.' => 'Κυρία.',
	'Dr.' => 'Δρ.',
	'Prof.' => 'Καθ.',
	'SINGLE_Cases'=>'Υπόθεση',
	'LBL_TAX_MODE_INFO' =>'If Individual, Taxes will be picked from Product record. If Group, Taxes will be picked from Global settings.',
	'LBL_CURRENCY_INFO' =>'Οι τιμές θα αλλάξουν στο επιλεγμένο συνάλλαγμα βάσει του ρυθμισμένου ποσοστού μετατροπών.',
	'LBL_TAX_REGION_INFO' =>'Για μεταβλητούς φόρους, το ποσοστό φόρου θα βασίζεται σε επιλεγμένη φορολογική περιοχή.',
	'LBL_TAX_CONFIGURE' =>'Ρύθμιση της φορολογικής περιοχής',
	'LBL_SEND_MSG_TO_SUPPORT' => 'Αποστολή email στην υποστήριξη',
	'LBL_CHAT_WITH_SUPPORT' => 'Ζωντανή συνομιλία με τον ειδικό του CRM',

	'LBL_ROLES' => 'Ρόλοι',
	'LBL_ROLEANDSUBORDINATE' => 'Ρόλοι και υπο-ρόλοι',
	'LBL_YOUR_LEARNING_SCORE' => 'Your CRM learning score',
	'LBL_SWITCH_TO_VTIGER7_NOW' => 'Switch to Vtiger7 by Feb 15th',
	'LBL_VTIGER6_RETIREMENT_NOTICE' => 'This version will be retired on Feb 15th 2016',
	'ModTracker' => 'Ενημερώσεις',
	'LBL_MAIL_SENT_SUCCESSFULLY' => 'Επιτυχής αποστολή',
	'LBL_FAILED_TO_SEND' => 'Αποτυχία αποστολής',

	'LBL_ESSENTIALS' => 'Απαραίτητα',
	'LBL_ALL_SETTINGS' => 'Όλες οι ρυθμίσεις',
	'On' => 'On',
	'LBL_OPEN_IN_NEW_TAB' => 'Ανοίξτε σε νέα καρτέλα',
	'LBL_DELETE_FOREVER'=>'Διαγράψτε για πάντα',
	'LBL_PLEASE_CLICK_ON_EDIT_TO_ADD_NOTE' =>'Κάντε κλικ στην επεξεργασία για να προσθέσετε σημειώσεις εδώ..',
	'LBL_PLEASE_ENTER_YOUR_NOTE' => 'Παρακαλώ εισάγετε τις σημειώσεις σας εδώ..',
	'LBL_PORTAL' => 'Δικτυακή Πύλη πελάτη',
	'LBL_SESSION' =>'Συνεδρία',
	'LBL_SESSIONS' => 'Συνεδρίες',
	'week days later' => 'εβδομάδες αργότερα',
	'more than week days later' => 'περισσότερες από εβδομάδες αργότερα',
	'less than week days later' => 'λιγότερο από εβδομάδες αργότερα',
	'week days ago' => 'πριν από εβδομάδες',
	'more than week days ago' => 'περισσότερες από εβδομάδες πριν',
	'less than week days ago' => 'λιγότερο από εβδομάδες πριν',

	'LBL_DUPLICATES' => 'Διπλότυπα',
	'LBL_DUPLICATES_DETECTED' => 'Βρέθηκε Διπλότυπο(α)!',
	'LBL_DUPLICATES_FOUND_MESSAGE' => 'Αυτό το %s δεν μπορεί να αποθηκευτεί αφού το διπλότυπο %s υπάρχει στο Vtiger με τις αντίστοιχες τιμές για το %s.',
	'LBL_VTIGER_NOTIFICATION' => 'Μήνυμα από: Ειδοποιήσεις συστήματος Vtiger',
	'LBL_DUPLICATION_FAILURE_FROM_WORKFLOWS' => 'Δεν ήταν δυνατή η αποθήκευση των ακόλουθων %s από τη ροή εργασίας "%s", καθώς ένα άλλο %s υπάρχει με τις ίδιες τιμές.',
	'LBL_DUPLICATION_FAILURE_FROM_WEBFORMS' => 'Δεν ήταν δυνατή η αποθήκευση των ακόλουθων %s από τη φόρμα ιστοσελίδας "%s", καθώς ένα άλλο %s υπάρχει με τις ίδιες τιμές.',
	'LBL_DUPLICATION_FAILURE_FOR_ADMIN' => 'Μπορείτε να %s τροποποιήσετε τους κανόνες πρόληψης διπλότυπων.',
	'LBL_DUPLICATION_FAILURE_FOR_NON_ADMIN' => 'Επικοινωνήστε με τον διαχειριστή σας για την αναθεώρηση των κανόνων για διπλότυπα',
	'LBL_DUPLICATE_RECORD_LISTS' =>'Διπλότυπο %s(α) :',
	'LBL_COLUMNS_ADDED'=>'%s προστέθηκαν στήλες',
	'LBL_SHOW_ALL'=>'Προβολή όλων',
	'LBL_LanguageEditor_WEBTITLE' => 'Επεξεργαστής ονομάτων πεδίων',
	'LBL_TAX_CALCULATIONS_WEBTITLE' => 'Φορολογική διαχείριση',
	'LBL_Tags_WEBTITLE' => 'Οι ετικέτες μου',
	'LBL_HIDE' => 'Απόκρυψη',
	'LBL_PRINT' => 'Εκτύπωση',
	'LBL_FORWARD' => 'Προώθηση',
	'LBL_NO_DEDUCTED_TAX'=>'Δεν έχουν αφαιρεθεί οι φόροι',
	'LBL_CLICK_TO_CONFIGURE'=>'Κάντε κλικ εδώ για να διαμορφώσετε',
	'LBL_KANBAN_VIEW_INFO' => 'Η προβολή Kanban δείχνει μόνο τις ευκαιρίες που αντιστοιχούν στις συνθήκες αυτής της λίστας',
	'LBL_CUSTOM_FIELDS_MAX_LIMIT' => 'Μπορείτε να προσθέσετε το μέγιστο %s από προσαρμοσμένα πεδία',
	'LBL_EXCEEDED_MODULE_RECORD_LIMIT' => '%s το όριο εγγραφών (%s) υπερέβη',
	'LBL_EXCEEDED_MODULE_RECORD_LIMIT_TEXT' =>'Διαπιστώσαμε ότι έχετε περισσότερες από %s εγγραφές στο %s. <br> Υπερβαίνει το μέγιστο όριο αυτής της έκδοσης. <br>Καταργήστε τις υπερβολικές εγγραφές και δοκιμάστε ξανά',
	'LBL_TEMPLATE_INFO_PART1'=>'Για να δείτε όλα τα πρότυπα ή τα πρότυπα άλλων ενοτήτων, εισαγάγετε το όνομα της ενότητας ή το "Όλα" στο πλαίσιο αναζήτησης Ενότητας.', 
	'LBL_TEMPLATE_INFO_PART2'=>'Εάν οι παραλήπτες στο πεδίο ΠΡΟΣ δεν είναι της ίδιας ενότητας με το πρότυπο, η παρακολούθηση δεν θα είναι ενεργοποιημένη για αυτούς τους παραλήπτες και τα πεδία συγχώνευσης δεν θα συγχωνευθούν.',
	'LBL_HIDDEN_BECAUSE_OF_DEPENDENCY' => 'Το πεδίο είναι κρυφό λόγω του κανόνα εξάρτησης',
	'MAILCHIMP_REVOKE_ACCESS_MESSAGE' => 'Κάνοντας κλικ σε αυτό θα καταργηθεί ο υπάρχων έλεγχος ταυτότητας MailChimp. Δεν θα διαγράψει εγγραφές που έχουν ήδη συγχρονιστεί.',
	// Added for Event Forms
	'LBL_VIEW_ANSWERS' => 'Προβολή απαντήσεων',
	'LBL_ANSWERS' => 'Απαντήσεις',

	//Default sort order translations
	'LBL_DEFAULT_SORT_ORDER' => 'Προκαθορισμένη στήλη ταξινόμησης και σειρά ταξινόμησης',
	'LBL_SELECT_COLUMN' => 'Επιλέξτε στήλη',
	'LBL_SELECT_SORT_ORDER' => 'Επιλέξτε σειρά ταξινόμησης',
	'LBL_ASCENDING' => 'Αύξουσα',
	'LBL_DESCENDING' => 'Φθίνουσα',
	'LBL_DEFAULT_SORT_MESSAGE' => 'Ο κατάλογος ταξινομείται πάντοτε από προεπιλογή σε αυτήν τη στήλη για κάθε νέα συνεδρία χρήστη.',
	'LBL_QUEUED' => 'Σε αναμονή',
	// End
	//Added for Inbox
	'LBL_EMAILS'=>'Emails',
	'LBL_CONVERSATIONS'=>'Συνομιλίες',
	'LBL_FROM_DISABLED_MESSAGE'=>'Ο διακομιστής αλληλογραφίας που έχει επιλεγεί στη σελίδα Ρυθμίσεις εξερχόμενου διακομιστή δεν επιτρέπει την αλλαγή της διεύθυνσης Από.',
	// End
	'LBL_MODULE_MANDATORY' => 'Η ενότητα είναι υποχρεωτική',
	'Online' => 'Online',
	'Busy' => 'Απασχολημένος',
	'LBL_BETA'=>'BETA',
	'JS_SHARED_WITH'=>'Μοιρασμένο με',
	'LBL_REPLY_TO' =>'Απάντηση σε',
	'LBL_MODULE_ASSIGNMENT_RULES' => '%s Κανόνες αντιστοίχισης',

	'LBL_RECENT_UPDATES' => 'Πρόσφατες ενημερώσεις',
	'LBL_VIEW_UPDATES_IN_DETAIL' => 'Διαβάστε περισσότερα ...',
	'LBL_DEFAULT_DASHBOARD_TOOLTIP'=>'Ορίστε αυτόν τον πίνακα ελέγχου ως προεπιλογή, ανακατατάσσοντάς τον ως την πρώτη καρτέλα!',
);

$jsLanguageStrings = array(
	'JS_EMAIL_SERVER_CONFIGURATION' => 'Παρακαλώ διαμορφώστε τις Ρυθμίσεις του εξερχόμενου διακομιστή από την σελίδα ΡΥΘΜΙΣΕΙΣ',
	'JS_SMS_SERVER_CONFIGURATION' => 'Παρακαλώ διαμορφώστε τις ρυθμίσεις των ειδοποιήσεων SMS από την σελίδα ΕΙΔΟΠΟΙΗΣΕΙΣ SMS',
	'JS_PLEASE_SELECT_ONE_RECORD' => 'Παρακαλώ επιλέξτε τουλάχιστον μία εγγραφή',
	//dropdown categary
	'JS_USERS' => 'Χρήστες',
	'JS_GROUPS' => 'Ομάδες',
	'JS_ALL' => 'Όλα',
	//Validation Error Message
	'JS_PLEASE_ENTER_VALID_VALUE' => 'Παρακαλώ εισάγετε μια έγκυρη τιμή',
	'JS_PLEASE_ENTER_VALID_TAG_NAME' => 'Παρακαλώ εισαγάγετε ένα έγκυρο όνομα ετικέτας',
	'JS_TAG_SAVED_SUCCESSFULLY' => 'Η ετικέτα αποθηκεύτηκε με επιτυχία',
	'JS_PLEASE_ENTER_VALID_EMAIL_ADDRESS' => 'Παρακαλώ Εισάγετε ένα έγκυρο Email',
	'JS_CONTAINS_ILLEGAL_CHARACTERS' => 'Περιέχει μη έγκυρους χαρακτήρες',
	'JS_PHONE_NUMBER_LENGTH_EXCEEDED' => 'Το μήκος τηλεφωνικού αριθμού υπερβαίνει το όριο',
	'JS_ACCEPT_POSITIVE_NUMBER' => 'Επιτρέπονται μόνο θετικοί αριθμοί',
	'JS_ACCEPT_WHOLE_NUMBERS' => 'Επιτρέπονται μόνο ακέραιοι αριθμοί',
	'JS_VALUE_SHOULD_BE_GREATER_THAN_ZERO' => 'Η τιμή πρέπει να είναι μεγαλύτερη από μηδέν',
	'JS_NUMBER_SHOULD_BE_LESS_THAN_32' => 'Ο αριθμός πρέπει να είναι μικρότερος από 32',
	'JS_PLEASE_ENTER_VALID_DATE' => 'Παρακαλώ εισαγάγετε μια έγκυρη ημερομηνία',
	'JS_PLEASE_ENTER_VALID_TIME' => 'Παρακαλώ εισαγάγετε μια έγκυρη ώρα',
	'JS_INVALID_PAGE_NUMBER' => 'Μη έγκυρος αριθμός σελίδας',
	'JS_SUBJECT_VALUE_CANT_BE_EMPTY' => 'Η υποκείμενη τιμή δεν μπορεί να είναι κενή',
	'JS_PERCENTAGE_SHOULD_BE_LESS_THAN_100' => 'Το ποσοστό πρέπει να είναι μικρότερο από 100',


	'INVALID_NUMBER_OF' => 'Μη έγκυρος αριθμός από',
	'INVALID_NUMBER' => 'Μη έγκυρος αριθμός',
	'JS_LBL_ARE_YOU_SURE_YOU_WANT_TO_DELETE' => 'Είστε σίγουρος/η ότι Θέλετε να διαγράψετε;',

	'OVERWRITE_EXISTING_MSG1' => 'Αντικατάσταση της διευθύνσεως με την επιλεγμένη; ',
	'OVERWRITE_EXISTING_MSG2' => 'στοιχεία διευθύνσεως;',
	'SINGLE_Accounts' => 'Εταιρεία',
	'SINGLE_Contacts' => 'Πελάτης',
	'SINGLE_Project' => 'Έργο',
	'SINGLE_ProjectMilestone' => 'Ορόσημο Έργου',

	'LBL_DELETE_CONFIRMATION' => 'Είστε βέβαιοι για την διαγραφή;', // This is added as duplicate as we need to show different message for different modules. Ex:Accounts has different message
	'LBL_UNLINK_CONFIRMATION' => 'Είστε βέβαιοι για την αποσύνδεση;',
	'LBL_DELETE_USER_CONFIRMATION' => 'Όταν ένας χρήστης διαγραφεί, ο χρήστης θα επισημανθεί ως "ανενεργός" και δεν θα μπορεί να ανατεθεί καμία νέα εγγραφή στο χρήστη και ο χρήστης δεν θα μπορέσει να συνδεθεί. Είστε βέβαιοι ότι θέλετε να διαγράψετε;',
	'LBL_DELETE_CONFIRMATION_NOTE' => 'Σημείωση: Οι διεγραμμένες εγγραφές θα φυλάσσονται στον κάδο ανακύκλωσης και θα καθαρίζονται μετά από %s ημέρες.',
	'LBL_MASS_DELETE_CONFIRMATION' => 'Είστε βέβαιοι ότι θέλετε να διαγράψετε τις επιλεγμένες εγγραφές;',
	'JS_LBL_SAVE' => 'Αποθήκευση',
	'JS_LBL_CANCEL' => 'Ακύρωση',
	'JS_DUPLICTAE_CREATION_CONFIRMATION' => 'Το όνομα εταιρείας υπάρχει ήδη. Θα θέλετε να δημιουργήσετε μια διπλότυπη εγγραφή;',

	'SHOULD_BE_LESS_THAN_TODAY' => 'Πρέπει να συμβεί πριν από σήμερα',
	'JS_PLEASE_SELECT_ATLEAST_ONE_OPTION' => 'Διαλέξτε τουλάχιστον μία επιλογή',
	'JS_SELECT_MODULE' => 'Παρακαλώ επιλέξτε ενότητα',
	'JS_PLEASE_SELECT_ATLEAST_ONE_MANDATORY_FIELD' => 'Παρακαλώ επιλέξτε τουλάχιστον ένα υποχρεωτικό πεδίο',
	'JS_SELECT_MODULE' => 'Παρακαλώ επιλέξτε μια ενότητα',
	'JS_PLEASE_ENTER_INTEGER_VALUE' => 'Παρακαλώ πληκτρολογήστε τιμή ακέραιου αριθμού',
	'JS_PLEASE_ENTER_DECIMAL_VALUE' => 'Παρακαλώ εισάγετε δεκαδική τιμή',
	'JS_PLEASE_ENTER_POSITIVE_DECIMAL_VALUE' => 'Παρακαλώ εισαγάγετε θετική δεκαδική τιμή',

	'JS_START_TIME_SHOULD_BE_GREATER_THAN_CURRENT_TIME' => 'Η ώρα έναρξης θα πρέπει να είναι μεγαλύτερη από την τρέχουσα ώρα',
	'JS_SHOULD_BE_LESS_THAN_CURRENT_DATE' => 'θα πρέπει να είναι μικρότερη από την τρέχουσα ημερομηνία',
	'JS_SHOULD_BE_GREATER_THAN_CURRENT_DATE' => 'θα πρέπει να είναι μεγαλύτερη από την τρέχουσα ημερομηνία',
	'JS_SHOULD_BE_GREATER_THAN_OR_EQUAL_TO' => 'πρέπει να είναι μεγαλύτερη ή ίση με',
	'JS_SHOULD_BE_LESS_THAN_OR_EQUAL_TO' => 'θα πρέπει να είναι μικρότερη ή ίση με',
	'JS_SHOULD_BE_GREATER_THAN_OR_EQUAL_TO' => 'πρέπει να είναι μεγαλύτερη ή ίση με',
	'JS_CURRENT_DATE' => 'Σημερινή ημερομηνία',
	'JS_PERCENTAGE_VALUE_SHOULD_BE_LESS_THAN_100' => 'Η εκατοστιαία τιμή πρέπει να είναι μικρότερη από 100',
	'JS_ACCEPT_ONLY_NUMBER' => 'Αποδέχεται μόνο αριθμούς',
	'JS_THIS_FILE_HAS_ALREADY_BEEN_SELECTED' => 'Αυτό το αρχείο έχει ήδη επιλεγεί',
	'JS_MAX_FILE_UPLOAD_EXCEEDS' => 'Το μέγιστο μέγεθος του αρχείου μεταφόρτωσης υπερβαίνει',
	'JS_THIS_DOCUMENT_HAS_ALREADY_BEEN_SELECTED' => 'Αυτό το έγγραφο έχει ήδη επισυναφθεί',
	'JS_LIST_EXIST' => 'Η λίστα υπάρχει ήδη',


	//Roles
	'JS_FAILED_TO_SAVE' => 'Αποτυχία αποθήκευσης αλλαγών στο διακομιστή',

	//Import Translations
	'JS_IMPORT_FILE_CAN_NOT_BE_EMPTY' => 'Το αρχείο εισαγωγής δεν μπορεί να είναι κενό',
	'JS_PLEASE_SELECT_ONE_FIELD_FOR_MERGE' => 'Επιλέξτε τουλάχιστον ένα πεδίο αντιστοίχισης',
	'JS_MAP_NAME_CAN_NOT_BE_EMPTY' => 'Το όνομα αντιστοίχισης δεν μπορεί να είναι κενό',
	'JS_MAP_NAME_ALREADY_EXISTS' => 'Το όνομα αντιστοίχισης υπάρχει ήδη',
	'JS_SELECT_FILE_EXTENSION' => 'Επιλέξτε ένα αρχείο με την ακόλουθη επέκταση:',
	'JS_UPLOADED_FILE_SIZE_SHOULD_BE_LESS_THAN' => 'Το μέγεθος του αρχείου μεταφόρτωσης πρέπει να είναι μικρότερο από',
	'JS_UPLOADED_FILE_SIZE_EXCEEDS' => 'Το μεταφορτωμένο μέγεθος αρχείου υπερβαίνει το',
	'JS_PLEASE_SPLIT_FILE_AND_IMPORT_AGAIN' => 'Διαχωρίστε το αρχείο σε μικρότερα αρχεία και εισάγετε ξανά.',
	'JS_FIELD_MAPPED_MORE_THAN_ONCE' => 'Πεδίο διασυνδεδεμένο περισσότερες από μία φορές',
	'JS_MAP_MANDATORY_FIELDS' => 'Παρακαλώ διασυνδέστε τα υποχρεωτικά πεδία',
	'JS_MAP_MERGE_CRITERIA_FIELDS' => 'Παρακαλούμε αντιστοιχίστε σχετιζόμενα πεδία',
	'JS_PLEASE_ENTER_SOME_VALUE' => 'Εισαγάγετε κάποια τιμή για αναζήτηση',
	'JS_VCF_FILE_IMPORT_WARNING' => 'Έχετε επιλέξει ένα αρχείο VCF. Μόνο οι επαφές μπορούν να εισαχθούν από αρχείο VCF. Για να εισαγάγετε αυτό το αρχείο, ξεκινήστε την εισαγωγή από την ενότητα Πελάτες',
	'JS_ICS_FILE_IMPORT_WARNING' => 'Έχετε επιλέξει ένα αρχείο ICS. Μόνο ημερολόγιο μπορεί να εισαχθεί από αρχείο ICS. Για να εισαγάγετε αυτό το αρχείο, ξεκινήστε την εισαγωγή από την ενότητα Ημερολόγιο',
	//Select2 elements validation message
	'JS_YOU_CAN_SELECT_ONLY'=> 'Μπορείτε να επιλέξετε μόνο',
	'JS_ITEMS' => 'αντικείμενα',

	'JS_LBL_PERMISSION' => 'Δικαιώματα πρόσβασης',
	'JS_CHECK_FILE_INTEGRITY' => 'Έλεγχος Ακεραιότητας Αρχείου',
	'JS_DUPLICATE_RECORD' => 'Διπλότυπη εγγραφή',

	//advance filter checkbox type condition values
	'JS_IS_ENABLED' => 'Είναι Ενεργό',
	'JS_IS_DISABLED' => 'Είναι Ανενεργό',

	'JS_PLEASE_ENTER_SOME_TEXT_FOR_COMMENT' => 'Παρακαλώ γράψτε κάποιο κείμενο για σχολιασμό',
	'JS_LBL_COMMENT_VALUE_CANT_BE_EMPTY' => 'Η τιμή του σχολιασμού δεν μπορεί να είναι κενή',
	'JS_REQUIRED_FIELD' => 'Αυτό το πεδίο είναι απαραίτητο',
	'JS_CAN_NOT_REMOVE_DEFAULT_WIDGET' => 'Δεν Μπορείτε να αφαιρέσετε το προεπιλεγμένο Widget',
	'JS_SHOULD_BE_LESS_THAN_CURRENT_DATE' => 'θα πρέπει να είναι μικρότερη από την τρέχουσα ημερομηνία',
	'JS_NO_CREATE_OR_NOT_QUICK_CREATE_ENABLED' => 'Άρνηση δικαιώματος δημιουργίας ή ανενεργή δυνατότητα γρήγορης δημιουργίας',

	//Image Action
	'LBL_IMAGE_DELETED_SUCCESSFULLY' => 'Η εικόνα διαγράφηκε επιτυχώς',
	'LBL_IMAGE_NOT_DELETED' => 'Η εικόνα δεν διαγράφηκε',
	'LBL_YES' => 'Ναι',
	'LBL_NO' => 'Όχι',
	'AM' => 'ΠΜ',
	'PM' => 'ΜΜ',
	'JS_ERROR' => 'Λάθος',
	'JS_INFORMATION' => 'Πληροφορία',
	'JS_DO_NOT_HAVE_AN_EMAIL_ID' => 'Δεν έχει Διευθ. Email',
	'NONE_OF_THE_FIELD_VALUES_ARE_CHANGED_IN_MASS_EDIT' => 'Καμία από τις τιμές πεδίων δεν έχουν αλλάξει στην Μαζική Επεξεργασία',

	//Notification messages for inventory module
	'JS_PLEASE_SELECT_AN_ACCOUNT_TO_COPY_ADDRESS' => 'Παρακαλώ επιλέξτε μια Εταιρεία για αντιγραφή της διεύθυνσης',
	'JS_PLEASE_SELECT_AN_CONTACT_TO_COPY_ADDRESS' => 'Παρακαλώ επιλέξτε Πελάτη για αντιγραφή τής διεύθυνσης',
	'JS_SELECTED_ACCOUNT_DOES_NOT_HAVE_AN_ADDRESS' => 'Η επιλεγμένη Εταιρεία δεν περιέχει διεύθυνση για αντιγραφή',
	'JS_SELECTED_CONTACT_DOES_NOT_HAVE_AN_ADDRESS' => 'Ο επιλεγμένος Πελάτης δεν περιέχει διεύθυνση για αντιγραφή',
	'JS_PLEASE_ENTER_PRIMARY_EMAIL_VALUE_TO_ENABLE_PORTAL_USER' => 'Παρακαλώ εισάγετε το Πρωτεύον Email για την ενεργοποίηση του χρήστη Δικτυακής Πύλης',
	'JS_PRIMARY_EMAIL_FIELD_DOES_NOT_EXISTS' => 'Το πρωτεύον Email δεν υπάρχει για την ενεργοποίηση του χρήστη Δικτυακής Πύλης.',

	//Tag Cloud validation messages
	'JS_PLEASE_ENTER_A_TAG' => 'Παρακαλώ εισάγετε μία Ετικέτα',
	'JS_TAG_NAME_ALREADY_EXIST' => 'Το όνομα της Ετικέτας υπάρχει ήδη',
	'JS_MAX_TAG_LENGTH_EXCEEDS' => 'Υπέρβαση ορίου χαρακτήρων Ετικέτας',

	'JS_NO_VIEW_PERMISSION_AFTER_SAVE'=> 'Δεν θα έχετε δικαίωμα προβολής αυτής της εγγραφής μετά την αποθήκευση. Επιθυμείτε να συνεχίσετε;',
	'JS_NO_RECORDS_RELATED_TO_THIS_FILTER' => 'Καμία εγγραφή δεν σχετίζετε με αυτό το Φίλτρο',

	// Calendar Reminder popup
	'JS_START_DATE_TIME' => 'Ημ. και Ώρα Έναρξης',
	'JS_END_DATE_TIME' => 'Ημ. και Ώρα Ολοκλήρωσης',
	'JS_POSTPONE' => 'Αναβολή',
	'JS_MESSAGE' => 'Μήνυμα',

	'JS_NO_VIEW_PERMISSION_AFTER_SAVE'=> 'Δεν θα έχετε δικαίωμα προβολής αυτής της εγγραφής μετά την αποθήκευση. Επιθυμείτε να συνεχίσετε;',

	//Settings page translations
	'JS_SUCCESSFULLY_PINNED' => 'Η συντόμευση προστέθηκε Επιτυχώς',
	'JS_SUCCESSFULLY_UNPINNED' => 'Η συντόμευση αφαιρέθηκε Επιτυχώς',
	'JS_SHORTCUT_ALREADY_ADDED' => 'Αυτή η Συντόμευση έχει ήδη προστεθεί',
	'JS_MESSAGE' => 'Μήνυμα',
	'JS_MASS_EDIT_LIMIT' => 'Η λειτουργία μαζικής επεξεργασίας μπορεί να γίνει σε 500 ή λιγότερες εγγραφές κάθε φορά',

	//Module Manager
	'JS_MODULE_ENABLED' => 'Η ενότητα Ενεργοποιήθηκε',
	'JS_MODULE_DISABLED' => 'Η ενότητα Απενεργοποιήθηκε',

	//Crom tasks
	'JS_VALUE_SHOULD_NOT_BE_LESS_THAN' => 'Η συχνότητα οποιασδήποτε εργασίας cron που διαμορφώνεται δεν πρέπει να είναι μικρότερη από',
	'JS_MINUTES' => 'λεπτά',

	//PickList
	'JS_ITEM_RENAMED_SUCCESSFULLY' => 'Το αντικείμενο μετονομάστηκε Επιτυχώς',
	'JS_ITEM_ADDED_SUCCESSFULLY' => 'Το αντικείμενο προστέθηκε Επιτυχώς',
	'JS_NO_ITEM_SELECTED' => 'Κανένα επιλεγμένο Αντικείμενο',
	'JS_MORE_THAN_ONE_ITEM_SELECTED' => 'Περισσότερα από ένα Αντικείμενα έχουν επιλεχθεί',
	'JS_ITEMS_DELETED_SUCCESSFULLY' => 'Το Αντικείμενο διαγράφτηκε επιτυχώς',
	'JS_DUPLICATE_ENTRIES_FOUND_FOR_THE_VALUE' => 'Διπλές καταχωρίσεις βρέθηκαν για την τιμή',
	'JS_SELECT_RECORDS_TO_MERGE_FROM_SAME_GROUP' => 'Πρέπει να επιλέξετε τις εγγραφές από τις ίδιες ομάδες για συγχώνευση',
	'JS_SELECT_ATLEAST_TWO_RECORD_FOR_MERGING' => 'Επιλέξτε τουλάχιστον δύο εγγραφές για συγχώνευση',
	'JS_ALLOWED_TO_SELECT_MAX_OF_THREE_RECORDS' => 'Το επιτρεπόμενο ανώτατο όριο επιλογής είναι Τρεις Εγγραφές',
	//Notification message for dashboard widget delete
	'JS_ARE_YOU_SURE_TO_DELETE_WIDGET' => 'Είστε Σίγουροι ότι θέλετε να διαγράψετε το widget - %s;',
	'JS_ARE_YOU_SURE_TO_DELETE_DASHBOARDTAB' => 'Είστε Σίγουροι ότι θέλετε να διαγράψετε αυτή την καρτέλα Πίνακα ελέγχου - %s;',
	'JS_TABS_LIMIT_EXCEEDED' => 'Συγνώμη! Δεν είναι δυνατή η προσθήκη περισσότερων από 10 καρτελών',
	'JS_DUPLICATE_ENTRIES_FOUND_FOR_THE_VALUE' => 'Διπλές καταχωρίσεις βρέθηκαν για αυτή την τιμή',
	'JS_SELECT_RECORDS_TO_MERGE_FROM_SAME_GROUP' => 'Για συγχώνευση πρέπει να επιλέξετε τις εγγραφές από τις ίδιες ομάδες',
	'JS_SELECT_ATLEAST_TWO_RECORD_FOR_MERGING' => 'Επιλέξτε τουλάχιστον δύο εγγραφές για συγχώνευση',
	'JS_ALLOWED_TO_SELECT_MAX_OF_THREE_RECORDS' => 'Έχετε τη δυνατότητα να επιλέξετε έως και τρεις εγγραφές',
	'JS_TOTAL_RECORDS' => 'Συνολικές Εγγραφές',
	'JS_PAGE_NOT_EXIST' => 'Η Σελίδα δεν Υπάρχει',
	'JS_YOU_ARE_IN_PAGE_NUMBER' => 'Βρίσκεστε στην σελίδα',

	//More currencies message
	'JS_PLEASE_ENABLE_BASE_CURRENCY_FOR_PRODUCT' => 'Παρακαλούμε ενεργοποιήστε το νόμισμα βάσης για το προϊόν',
	'JS_PLEASE_SELECT_BASE_CURRENCY_FOR_PRODUCT' => 'Επιλέξτε νόμισμα βάσης για το προϊόν',
	'JS_BASE_CURRENCY_CHANGED_TO_DISABLE_CURRENCY' => 'Το βασικό συνάλλαγμα πρέπει να αλλάξει για να απενεργοποιηθεί ',

	//Delete Records message
	'JS_RECORDS_ARE_GETTING_DELETED' => 'Οι εγγραφές αρχίζουν να Διαγράφονται',
	'JS_RECORD_GETTING_DELETED' => 'Η εγγραφή αρχίζει να Διαγράφεται',
	'JS_NO_RESULTS_FOUND' => 'Δεν βρέθηκαν αποτελέσματα',

	'JS_RECORDS TRANSFERRED SUCCESSFULLY' => 'Τα δικαιώματα των εγγραφών έχουν μεταφερθεί με επιτυχία',

	//TextSearch Language String
	'LBL_SEARCHING' => 'Αναζήτηση σε ',
	'JS_RECORDS_TRANSFERRED_SUCCESSFULLY' => 'Τα δικαιώματα των εγγραφών έχουν μεταφερθεί με επιτυχία.',
	'JS_USER_DELETED_SUCCESSFULLY' => 'Ο χρήστης διαγράφηκε επιτυχώς.',

	'JS_MAX_RECORDS_LIMIT' => 'Μέγιστες εγγραφές για εκτύπωση είναι :',
	'JS_USER_EXISTS' => 'Ο Χρήστης Υπάρχει ήδη',
	'JS_REENTER_PASSWORDS' => 'Επανεισάγετε τα συνθηματικά',
	'LBL_SIGN_IN_AS_USER'=>'Αν συνδεθείτε ως ένας άλλος χρήστης θα τερματιστεί η τρέχουσα συνεδρία. Είστε βέβαιοι ότι θέλετε να συνεχίσετε;',
	'JS_REENTER_PASSWORDS' => 'Επανεισάγετε τα συνθηματικά',
	'SINGLE_Vendors' => 'Προμηθευτής',

	//Feedback on removing old version
	'JS_PLEASE_GIVE_YOUR_FEEDBACK' => 'Συμπληρώστε τα σχόλιά σας',
	'JS_PBX_CALL_FROM' => 'Κλήση από',
	'JS_PBX_INCOMING_CALL' => 'Εισερχόμενη κλήση',
	'JS_PBX_OUTGOING_SUCCESS'=>'Σηκώστε τον δέκτη επέκτασης για να καλέσετε τον αριθμό',
	'JS_PBX_OUTGOING_FAILURE'=>'Η κλήση απέτυχε',
	'JS_PBX_FILL_ALL_FIELDS'=>'Παρακαλώ συμπληρώστε όλα τα πεδία',
	'JS_PBX_CAMPAIGN_NAME' => 'Όνομα καμπάνιας',
	'JS_PBX_CAMPAIGN_NUMBER' => 'Αριθμός καμπάνιας',
	'JS_MIN_SEARCH_KEY_LENGTH' => 'Το κλειδί αναζήτησης πρέπει να έχει τουλάχιστον 3 χαρακτήρες',
	'JS_NO_MATCH' => 'Δεν βρέθηκαν σχετικές εγγραφές',
	'JS_PBX_OUTGOING_CALL' => 'Εξερχόμενη κλήση',
	'JS_PBX_CALL_TO' => 'Καλέστε στο',
	'JS_REQUIRED' => 'Απαιτείται',
	'JS_PHONE_NUMBER' => 'Αριθμός τηλεφώνου',
	'JS_NO_CREATE_PERMISSION' => 'Η κλήση πρέπει να απαντηθεί πριν από τη δημιουργία εγγραφής',
	'HelpDesk' => 'Αιτήματα',
	'Potentials' => 'Ευκαιρίες',
	'JS_CALL_NOT_ANSWERED_UPDATE_PERMISSION' => "Η κλήση πρέπει να απαντηθεί πριν συνδέσετε την εγγραφή", 
	'JS_CALL_NOT_ANSWERED' => "Η κλήση πρέπει να απαντηθεί πριν από την εκτέλεση της ενέργειας",
	// ends
	'JS_NO_EDIT_PERMISSION' => 'Δεν έχετε τα δικαιώματα για να επεξεργαστείτε αυτό το συμβάν',

	//Google Synchronization
	'SYNC_REMOVED_SUCCESSFULLY' => 'Ο συγχρονισμός Google αφαιρέθηκε με επιτυχία.',
	'REMOVE_SYNCHRONIZATION_MESSAGE' => 'Κάνοντας κλικ σε αυτό θα καταργηθεί η υπάρχουσα πιστοποίηση  Gmail. Δεν θα διαγράψει αρχεία που έχουν ήδη συγχρονιστεί.',
	'REMOVE_SYNCHRONIZATION' => 'Κατάργηση συγχρονισμού',
	'FIELD_MAPPING' => 'Διασύνδεση πεδίων',
	'LBL_SYNC_BUTTON' => 'Συγχρονισμός τώρα',
	'LBL_SYNCRONIZING' => 'Συγχρονισμός....',
	'LBL_NOT_SYNCRONIZED' => 'Δεν έχετε συγχρονίσει ακόμα',
	'LBL_FIELD_MAPPING' => 'Διασύνδεση πεδίων',

	'LBL_RESTORE_CONFIRMATION' => 'Είστε βέβαιοι ότι θέλετε να επαναφέρετε αυτόν τον χρήστη;',
	'LBL_DELETE_USER_PERMANENT_CONFIRMATION' => 'Μόλις διαγραφεί αυτός ο χρήστης δεν μπορεί να αποκατασταθεί. Είστε βέβαιοι ότι θέλετε να διαγράψετε οριστικά αυτόν τον χρήστη;',

	'LBL_LANGUAGE_LABEL_VALIDATION' => 'Η τιμή του ονόματος πεδίου δεν μπορεί να είναι κενή. Πρέπει να εισαγάγετε ένα όνομα.',
	'JS_CHANGES_WILL_BE_LOST' => 'Οι αλλαγές σας θα χαθούν!',
	'JS_RECORD_DELETED' => 'Η εγγραφή διαγράφηκε',
	'JS_NO_DELETE_PERMISSION' => 'Δεν υπάρχει δικαίωμα διαγραφής',
	'JS_CONFIRM_MARK_AS_HELD' => 'Είστε βέβαιοι ότι θέλετε να επισημάνετε το συμβάν ως Διεξήχθη?',
	'JS_CONFIRM_MARK_AS_COMPLETED' => 'Είστε βέβαιοι ότι θέλετε να επισημάνετε την εργασία ως Ολοκληρωμένη?',
	'JS_TODO_MARKED_AS_COMPLETED' => 'Η εργασία έχει επισημανθεί ως Ολοκληρωμένη',
	'JS_EVENT_MARKED_AS_HELD' => 'Το συμβάν έχει επισημανθεί ως Διεξήχθη',
	'JS_FUTURE_EVENT_CANNOT_BE_MARKED_AS_HELD' => 'Το μελλοντικό συμβάν δεν μπορεί να επισημανθεί ως Διεξήχθη',
	'JS_PERMISSION_DENIED' => 'Άρνηση πρόσβασης',
	'LBL_SMS_MAX_CHARACTERS_ALLOWED' => 'Για μηνύματα κειμένου επιτρέπονται το μέγιστο 160 χαρακτήρες',
	'JS_VIEWNAME_ALERT' => 'Για το όνομα της λίστας επιτρέπονται έως και 40 χαρακτήρες',
	'JS_EXCEEDS_MAX_UPLOAD_SIZE' => 'Υπέρβαση μέγιστου μεγέθους μεταφόρτωσης',
	'JS_FUTURE_EVENT_CANNOT_BE_HELD' => 'Δεν μπορεί να διεξαχθεί για το μέλλον',
	'LBL_WARNING' => 'Προειδοποίηση',
	'DELETE_USER_PERMANENT_WARNING' => 'Η διαγραφή ενός χρήστη μόνιμα θα μεταφέρει όλες τις εγγραφές, συμπεριλαμβανομένων των σχολίων και του ιστορικού, σε νέο χρήστη.',
	'JS_DECIMAL_SEPARATOR_AND_GROUPING_SEPARATOR_CANT_BE_SAME' => 'Ο δεκαδικός διαχωριστής και ο διαχωρισμός ομαδοποίησης δεν μπορούν να είναι ίδιοι',
	'JS_FUTURE_EVENT_CANNOT_BE_HELD' => 'Δεν μπορεί να διεξαχθεί για το μέλλον',
	'JS_CLOSE' => 'Κλείσιμο',

	// For Line Iteam Currency Symbol select while Import
	'JS_CURRENCY_TOOLTIP_CONTENT_TITLE' => 'Στοιχεία γραμμής συναλλάγματος',
	'JS_CURRENCY_TOOLTIP_CONTENT_MESSAGE' => 'Για την εξαγωγή τιμών συναλλάγματος στα στοιχεία γραμμής, το Vtiger θα χρησιμοποιήσει αυτήν τη ρύθμιση για εξαγωγή ως αυτό το συνάλλαγμα. Οποιεσδήποτε τιμές συναλλάγματος εκτός των στοιχείων γραμμής πινάκων, θα εξάγονται όπως με βάση το προτιμώμενο συνάλλαγμα του χρήστη (Οι προτιμήσεις μου)',
	'LBL_CANT_SELECT_CONTACT_FROM_LEADS' => 'Δεν είναι δυνατή η επιλογή των σχετικών Επαφών για Δυνητικούς',
	'JS_TRY_LATER' => 'Παρακαλώ δοκιμάστε αργότερα!',
	'JS_FILE_NAME' => 'Ονομα αρχείου',
	'JS_EXTERNAL_FILE_URL' => 'Url Αρχείου',
	'JS_PLEASE_SELECT_A_FILE' => 'Επιλέξτε ένα αρχείο',
	// SMSNotifier Phone Format Warning
	'JS_PHONEFORMAT_ERROR' => 'Ελέγξτε ότι ο αριθμός τηλεφώνου βρίσκεται σε διεθνή μορφή E.164 με πρόθεμα \'+\' και τον κωδικό χώρας παράδειγμα +1 415 599 2671',

	// Leads and Potentials Field Mapping
	'JS_NONE' => 'Κανέα',
	'JS_IS_ALREADY_BEEN_MAPPED' => 'διασυνδεδεμένο',
	'JS_CANT_MAP' => 'Δεν είναι δυνατή η διασύνδεση',
	'JS_WITH' => 'με',
	'JS_MAPPING_DELETED_SUCCESSFULLY' => 'Η διασύνδεση διαγράφηκε επιτυχώς',

	//JS Field Type Translation
	'JS_phone' => 'Τηλέφωνο',
	'JS_picklist' => 'Λίστα επιλογών',
	'JS_email' => 'Email',
	'JS_text' => 'Αλφαριθμητικό',
	'JS_currency' => 'Νόμισμα',
	'JS_multiSelectCombo' => 'Σύνθετο πλαίσιο Πολλαπλών επιλογών',
	'JS_time' => 'Ώρα',
	'JS_textArea' => 'Περιοχή κειμένου',
	'JS_url' => 'Σύνδεσμος Url',
	'JS_string' => 'Αλφαριθμητικό',
	'JS_checkBox' => 'Πλαίσιο ελέγχου',
	'JS_date' => 'Ημερομηνία',
	'JS_decimal' => 'Δεκαδικός',
	'JS_percent' => 'Ποσοστό',
	'JS_skype' => 'Skype',
	'JS_None' => 'Κανένα',
	'JS_integer' => 'Ακέραιος',
	'JS_double' => 'Δεκαδικός',
	'JS_reference' => 'αναφορά',
	'JS_image' => 'Εικόνα',

	// AutoFill Overwriting message
	'JS_OVERWRITE_AUTOFILL_MSG1' => 'Θα θέλατε να αντικαταστήσετε υπάρχοντα',
	'JS_OVERWRITE_AUTOFILL_MSG2' => 'με τα επιλεγμένα',

	//Demo Data Module Translatios
	'JS_ADD_DEMO_DATA_CONFIRMATION'=> 'Are you sure you want to add demo records to all modules?',
	'JS_ADD_DEMO_DATA_INFO'=> 'Demo records can be deleted by clicking the "Clear Demo Data" button.',
	'JS_CLEAR_DEMO_DATA_CONFIRMATION'=> 'Are you sure you want to clear demo data in all modules?',
	'JS_CLEAR_DEMO_DATA_INFO'=> 'Demo records can be added by clicking the "Add Demo Data" button.',
	'JS_DEMO_DATA_GETTING_POPULATED'=> 'Demo Data is being added, please wait...',
	'JS_DEMO_DATA_GETTING_CLEARED'=> 'Demo data is being cleared, please wait...',
	'JS_DEMO_DATA_ADDED' => 'Demo data added to all modules.',
	'JS_DEMO_DATA_CLEARED' => 'Demo data cleared from all modules.',
	'JS_HOME_DASH_BOARD_CLEAR_INFO'=>'Note: You can re-enable demo data from',
	'JS_SETTINGS_PATH'=>'Settings > Configuration > Configuration Editor.',
	'JS_PRINT' => 'Εκτύπωση',
	'JS_SAVE_AS_IMAGE' => 'Αποθήκευση ως εικόνα',
	'JS_DIRECTORY_IS_EMPTY' => 'Ο κατάλογος είναι κενός',
	'JS_PLEASE_SELECT_AN_RELATED_TO_COPY_ADDRESS' => 'Επιλέξτε ενότητα Δυνητικού/Πελάτη για να αντιγράψετε τη διεύθυνση',
	'SINGLE_Leads' => 'Δυνητικός',
	'JS_SELECTED_LEAD_DOES_NOT_HAVE_AN_ADDRESS' => 'Ο επιλεγμένο δυνητικός δεν περιέχει διεύθυνση για αντιγραφή',
	'JS_INVALID_EMAILS' => 'Μη έγκυρα Emails',
	'JS_DIRECTORY_IS_EMPTY' => 'Ο κατάλογος είναι κενός',
	'JS_NO_LINE_ITEM' =>'Τα στοιχεία γραμμής δεν πρέπει να είναι κενά.',
	'JS_INTERNAL_COMMENT' => 'Εσωτερικό σχόλιο',
	'JS_INTERNAL_COMMENT_INFO' => 'Το σχόλιο θα προβληθεί μόνο από τους χρήστες του CRM, εάν είναι ενεργοποιημένο το πλαίσιο ελέγχου εσωτερικού σχολιασμού. Για να ενημερώσετε τους πελάτες (μέσω διαμορφωμένων ροών εργασίας ή πύλης πελατών) αφήστε αυτό το πλαίσιο ελέγχου απενεργοποιημένο',

	'JS_NO_TAXES_EXISTS' => 'Δεν υπάρχουν φόροι',
	'JS_CONFIRM_TAXES_AND_CHARGES_REPLACE' => 'Θέλετε να ενημερώσετε τις τιμές φόρου και χρέωσης που αντιστοιχούν στην επιλεγμένη περιοχή;',
	'JS_PRINTTEMPLATEMODULE_DISABLED' => 'Η ενότητα PrintTemplates είναι απενεργοποιημένη. Δεν είναι δυνατή η εκτέλεση της λειτουργίας',
	'JS_SENT_FOR_SIGNATURE' => 'Απεστάλη προς υπογραφή',
	'JS_FAILED_TO_SEND_FOR_SIGNATURE' => 'Αποτυχία αποστολής για υπογραφή',
	'JS_SUITABLE_VTIGER_FIELD_NOT_AVAILABLE_FOR_MAPPING' => 'Δεν είναι διαθέσιμο κάποιο κατάλληλο πεδίο vtiger  για διασύνδεση',
	'JS_SAVED_SUCCESSFULLY' => 'Οι ρυθμίσεις αποθηκεύτηκαν με επιτυχία',
	'phone' => 'Τηλέφωνο',
	'email' => 'Email',
	'url' => 'Σύνδεσμος Url',
	'home' => 'Οικίας',
	'work' => 'Εργασίας',
	'custom' => 'Προσαρμοσμένο',
	'mobile' => 'Κινητό',
	'main' => 'Κύριο',
	'work_fax' => 'Φαξ',
	'home_fax' => 'Φαξ Οικίας',
	'pager' => 'Pager',
	'profile' => 'Προφίλ',
	'blog' => 'Blog',
	'home-page' => 'Αρχική σελίδα',
	'address' => 'Διεύθυνση',

	// Cron
	'JS_NONE_OF_THE_VALUES_ARE_CHANGED' => 'Καμία από τις τιμές δεν άλλαξε',
	'JS_REMOVE_SYNC_CONFIRMATION' => 'Η κατάργηση του συγχρονισμού θα καταργήσει μόνο τον υπάρχοντα έλεγχο ταυτότητας, δεν θα διαγράψει συγχρονισμένες εγγραφές. Αν συγχρονίσετε ξανά με τον ίδιο λογαριασμό Google, θα δημιουργηθούν διπλές εγγραφές. Είστε βέβαιοι ότι θέλετε να καταργήσετε συγχρονισμό;',

	//Recipient preferences labels
	'JS_LBL_GETTING_RECIPIENT_PREFS' => 'Λήψη προτιμήσεων παραλήπτη..',
	'JS_LBL_SAVING_PREFERENCES' => 'Αποθήκευση προτιμήσεων..',
	// Social Module Strings
	'JS_PLEASE_WAIT' => 'Παρακαλώ περιμένετε...',
	'JS_SOCIAL_FIELD_ERROR' => 'πεδίο απαραίτητο',
	// Twitter Specific
	'JS_ADD_FAVOURITE_SUCCESS_MSG' => 'Added Tweet to favourite list successfully',
	'JS_UNDO_FAVOURITE_SUCCESS_MSG' => 'Removed Tweet from favourite list successfully',
	'JS_UNDO_RETWEET_SUCCESS_MSG' => 'UndoRetweet successfully',
	'JS_STATUS_UPDATE_SUCCESS_MSG' => 'Updated successfully',
	'JS_TWEET_DELETE_MSG' => 'Status Deleted successfully',
	'JS_INVALID_TWITTER_HANDLER_NAME' => 'Invalid Twitter Username',
	'JS_RELATION_ADDED_SUCESSFULLY' => 'Η σχέση έχει προστεθεί με επιτυχία',
	'JS_FAILED_TO_ADD_RELATION' => 'Αποτυχία προσθήκης συσχέτισης',
	'JS_SOCIAL_POST_DELETE_CONFIRMATION' => 'Είστε βέβαιοι ότι θέλετε να διαγράψετε;',
	'JS_EVENT_OR_TODO_ADDED_SUCCESS_MSG' => 'Το συμβάν/εργασία προστέθηκε με επιτυχία',
	'JS_LEAD_ADDED_SUCCESS_MSG' => 'Ο δυνητικός προστέθηκε με επιτυχία',
	'JS_OPPORTUNITY_ADDED_SUCESS_MSG' => 'Η ευκαιρία προστέθηκε με επιτυχία',
	'JS_TICKET_ADDED_SUCESS_MSG' => 'Το αίτημα προστέθηκε με επιτυχία',
	'JS_CASE_ADDED_SUCESS_MSG' => 'Η υπόθεση προστέθηκε με επιτυχία',
	'JS_SELECT_OPTION' => 'Διαλέξτε μια επιλογή',

	'JS_LOADING_PLEASE_WAIT' => 'Φορτώνει παρακαλώ περιμένετε..',
	'JS_SAVING_PLEASE_WAIT' => 'Αποθήκευση ... Παρακαλώ περιμένετε ...',
	'JS_DUPLICATE_EMAILS_PLEASE_CHECK' => 'Διπλότυπο email. Παρακαλώ ελέγξτε',
	'JS_USER_EXISTS' => 'Ο χρήστης υπάρχει ήδη',
	'JS_ALERT' => 'Ειδοποίηση',
	'JS_SUCCESS' => 'Επιτυχία',
	'JS_REMOVE_MASS_TAG_WARNING' => 'Είστε βέβαιοι ότι θέλετε να καταργήσετε την ετικέτα %s για επιλεγμένες εγγραφές',
	'JS_VIEW_ALREADY_EXISTS' => 'Η προβολή υπάρχει ήδη',
	'JS_PLEASE_ENTER_NUMBER_IN_RANGE' => 'Παρακαλούμε εισάγετε τιμή στην περιοχή',
	'JS_UPLOAD' => 'Μεταφόρτωση',
	'JS_TODO_ADDED_SUCCESS_MSG' => 'Η εργασία προστέθηκε με επιτυχία',
	'JS_EVENT_ADDED_SUCCESS_MSG' => 'Το συμβάν προστέθηκε με επιτυχία',
	'JS_ACTIVE' => 'Ενεργό',
	'JS_SNOOZED' => 'Αναβλήθηκε',
	'JS_INVALID_URL' => 'Μη έγκυρο URL',
	'JS_MAX_ALLOWED_CHARACTERS' => 'Οι μέγιστοι επιτρεπόμενοι χαρακτήρες είναι',
	'JS_ARE_YOU_SURE_YOU_WANT_TO_DELETE' => 'Είστε βέβαιοι ότι θέλετε να διαγράψετε;',
	'JS_TAB_LABEL_EXCEEDS_CHARS' => 'Το μήκος του ονόματος πεδίου καρτελών θα πρέπει να είναι μικρότερο από %s χαρακτήρες',
	'JS_STARRED' => 'Μην ακολουθήσετε',
	'JS_NOT_STARRED' => 'Ακολουθηστε',
	//Tags
	'JS_NO_TAG_EXISTS' => 'Δεν υπάρχει ετικέτα' ,
	//help wizard
	'JS_HELP_AND_SUPPORT' => 'Βοήθεια και υποστήριξη',
	'JS_WHAT_DO_YOU_NEED_HELP_WITH' => 'Σε τι χρειάζεσαι βοήθεια;',
	'JS_SUBMIT' => 'Υποβολή',

	/** Vtiger7 translations **/
	'JS_ADD_MAX_15_ITEMS' => 'Μπορείτε να προσθέσετε μόνο 15 στοιχεία',
	'SINGLE_SLA' => 'Συμφωνία σε Επίπεδο Υπηρεσιών (SLA)',

	'JS_DEFAULT_ENABLED' => 'Προεπιλογή Ενεργή',
	'JS_DEFAULT_DISABLED' => 'Προεπιλογή Απενεργοποιημένη',
	'JS_EDIT_FIELDS' => '%s Πεδία & Διάταξη',
	'JS_MORE_VTIGER' => 'Υπάρχουν περισσότερες εγγραφές για συγχρονισμό στο vtiger',
	'JS_MORE_GOOGLE' => 'Υπάρχουν περισσότερες εγγραφές για συγχρονισμό στο Google',
	'JS_MORE_OFFICE365' => 'Υπάρχουν περισσότερες εγγραφές για συγχρονισμό στο Office365',
	'JS_TOKEN_EXPIRED' => 'Office365 Authentication has expired. Click on Sign in with Office365 button to re-authorize.',
	'JS_EDIT_FIELDS' => '%s Πεδία & Διάταξη',
	'JS_WISH_TO_PROCEED' =>'Είστε βέβαιοι ότι θέλετε να προχωρήσετε;',
	'JS_SWITCH_TO_OLD_LOOK_CONFIRMATION_MESSAGE' => 'Some features, including notifications, picklist colors, follow record, are not available in old version. You may switch back to new version by choosing new version from top right menu.',
	'JS_PROCEED_OLD_VERSION' => 'Proceed to old version',
	'JS_STAY_WITH_NEW_VERSION' => 'Stay with new version',

	//TimeAgo plugin Strings
	'JS_AGO' => 'πριν από',
	'JS_FROM_NOW' => 'από τώρα',
	'JS_LESSTHAN_A_MINUTE' => 'λιγότερο από ένα λεπτό',
	'JS_A_MINUTE' =>'λεπτό',
	'JS_MINUTES' => 'λεπτά',
	'JS_AN_HOUR' => 'ώρα',
	'JS_HOURS' => 'ώρες',
	'JS_A_DAY' => 'μέρα',
	'JS_DAYS' => 'μέρες',
	'JS_A_MONTH' => 'μήνας',
	'JS_MONTHS' => 'μήνες',
	'JS_A_YEAR' => 'χρόνος',
	'JS_YEARS' => 'χρόνια',
	'JS_TODAY' => 'σήμερα',
	'JS_TOMORROW' => 'αύριο',
	'JS_YESTERDAY' => 'εχθές',

	'JS_CREATE_TASK' => 'Δημιουργία εργασίας',

	//Notifications
	'JS_MENTION_ATLEAST_ONE_USER' => 'Αναφέρετε τουλάχιστον έναν χρήστη',
	'JS_LOADING' => 'Φόρτωση...',
	'JS_CASE_ACCEPTED_SUCCESSFULLY' => 'Η υπόθεση έγινε δεκτή με επιτυχία',

	'JS_UPLOAD_SUCCESSFUL' => 'Το αρχείο μεταφορτώθηκε επιτυχώς',
	'JS_UPLOAD_FAILED' => 'Απέτυχε η μεταφόρτωση',
	'JS_FAILED_TO_UPLOAD_FILE' => 'Απέτυχε η μεταφόρτωση αρχείου',
	'JS_DOCUMENT_CREATED' => 'Δημιουργήθηκε το αρχείο',
	'JS_DOCUMENT_CREATION_FAILED' => 'Η δημιουργία Αρχείου απέτυχε',

	'JS_TEMPLATE_TEXT_EXCEEDS_LIMIT' => 'Το μέγεθος του προτύπου υπερβαίνει το μέγιστο επιτρεπόμενο μέγεθος των 100 KB. Αφαιρέστε κάποιο κείμενο στο πρότυπο και δοκιμάστε ξανά',
	'JS_PLEASE_SELECT_IMAGE' => 'Επιλέξτε μια εικόνα',

	'JS_LBL_MY_PREFERENCES' => 'Οι προτιμήσεις μου',
	'JS_LBL_MY_PREFERENCES_TEXT' => 'Διαμορφώστε τις προσωπικές προτιμήσεις σας, συμπεριλαμβανομένης της ζώνης ώρας, του νομίσματος και της μορφής ημερομηνίας. Μπορείτε επίσης να ανεβάσετε εικόνα προφίλ από τις Προτιμήσεις μου.',
	'JS_LBL_HELP' => 'Βοήθεια',
	'JS_LBL_HELP_TEXT' => 'Κάντε κλικ στο κουμπί Βοήθεια για να βρείτε Οδηγούς και Συχνές Ερωτήσεις και χρησιμοποιήστε το αναπτυσσόμενο μενού Βοήθεια για να λάβετε βοήθεια από την ομάδα υποστήριξης του Vtiger',
	'JS_LBL_RESTART_TOUR' => 'Restart Guided Tour',
	'JS_LBL_RESTART_TOUR_TEXT' => 'Click here to start this tour again',
	'JS_GOOGLE_REMOVE_SYNC_CONFIRMATION' => 'Η κατάργηση της πρόσβασης θα καταργήσει μόνο τον υπάρχοντα έλεγχο ταυτότητας Google για τις Επαφές και το Ημερολόγιο, αλλά δεν θα διαγράψει συγχρονισμένες εγγραφές. Αν συγχρονίσετε ξανά με τον ίδιο λογαριασμό Google, θα δημιουργηθούν διπλές εγγραφές. Είστε βέβαιοι ότι θέλετε να ακυρώσετε την πρόσβαση;',
	'JS_OFFICE_REMOVE_SYNC_CONFIRMATION' => 'Revoke access will only remove existing Office365 authentication for Contacts, Calendar and Emails, it will not delete synchronized records. If you synchronize with same Office365 account again, duplicate records will be created. Are you sure you want to revoke access?',
	'JS_FOLLOW_RECORD' => 'Παρακολούθηση εγγραφής',
	'JS_FOLLOWING' => 'Παρακολούθηση',
	'JS_UNFOLLOW_RECORD' => 'Μη παρακολούθηση εγγραφής',
	'JS_UNFOLLOWING' => 'Μη παρακολούθηση',
	'JS_RECORD_CREATED' => 'Η εγγραφή δημιουργήθηκε',
	'JS_RECORD_UPDATED' => 'Η εγγραφή ενημερώθηκε',
	'JS_LIST_SAVED' => 'Η λίστα αποθηκεύτηκε',
	'JS_TAB_DELETED' => 'διεγράφη',
	'JS_TAB_RENAMED' => 'Μετονομάστηκε',
	'JS_TAB_REORDERED' => 'Reordered',
	'JS_RECIPIENT_PREFERENCES_SAVED' => 'Οι προτιμήσεις του παραλήπτη αποθηκεύτηκαν',
	'JS_TASK_ADDED' => 'Προστέθηκε εργασία',
	'JS_MARKED_COMPLETED' => 'Επισημάνθηκε ως ολοκληρωμένη',
	'JS_TASK_PRIORITY_CHANGED' => 'Η προτεραιότητα εργασίας τροποποιήθηκε',
	'JS_NO_PERMISSION_TO_MARK_AS_HELD' => 'Δεν έχετε δικαίωμα να επισημάνετε αυτό το γεγονός ως διεξήχθη',
	'JS_TAG_ADDED' => 'προστέθηκε Ετικέτα',
	'JS_TAB_DELETED' => 'διεγράφη',
	'JS_TAB_RENAMED' => 'Μετονομάστηκε',
	'JS_TAB_REORDERED' =>'Reordered',
	//Convert Lead
	'JS_CONVERT_LEAD_ERROR' => 'Πρέπει να ενεργοποιήσετε είτε την ενότητα Οργανισμοί ή Πελάτες για να μετατρέψετε τους Δυνητικούς.',

	'JS_CHECK_START_AND_END_DATE'=>'Η ημερομηνία και η ώρα ολοκλήρωσης θα πρέπει να είναι μεγαλύτερες ή ίσες με την Ημερομηνία και ώρα έναρξης',
	'JS_CHECK_START_AND_END_DATE_SHOULD_BE_GREATER'=> 'Η ημερομηνία και ώρα ολοκλήρωσης θα πρέπει να είναι μεγαλύτερες από την Ημερομηνία και ώρα έναρξης',
	'JS_HOLD_FOLLOW_UP_SHOULD_BE_LESS_THAN_CURRENT_DATE'=> 'Το Followup θα πρέπει να είναι μεγαλύτερό ή ίσo με την Ημερομηνία και ώρα έναρξης',
	'JS_DUPLICATES_DETECTED' => 'Εντοπίστηκαν διπλότυπα!',

	'JS_IS' => 'είναι',
	'JS_IS_NOT' => 'δεν είναι',
	'JS_EQUALS' => 'ίσο με',
	'JS_NOT_EQUALS_TO' => 'όχι ίσο με',
	'JS_ONLY_LETTERS_AND_NUMBERS_ALLOWED' => 'Μόνο γράμματα (a-z), αριθμοί (0-9) και κενά επιτρέπονται',
	'JS_WIDGET_RESIZING_WAIT_MSG' => 'Τα περιεχόμενα του widget θα φορτωθούν μετά την αλλαγή μεγέθους.',
	'JS_SLA_FULFILLMENT_RATE' => 'SLA FULFILLMENT RATE',
	'JS_TICKET_COUNT' => 'καταμέτρηση περιπτώσεων',
	'JS_CASES_AGE_HOURS'=>'ηλικία υπόθεση (ώρες)',
	'JS_BACKLOG'=> 'Backlog',
	'JS_AND' => 'και',
	'JS_SEQUENCE_UPDATED_SUCCESSFULLY'=>'Η ακολουθία ενημερώθηκε με επιτυχία',
	'JS_VIEW_RECORD' => 'Προβολή εγγραφής',
	'JS_HIDE'=>'Απόκρυψη',
	'JS_SHOW_ALL'=>'Προβολή όλων',
	'JS_SHOW_LESS' => 'Δείτε λιγότερα',
	'JS_SHOW_MORE' => 'Δείτε περισσότερα',

	'JS_KANBAN_ANCHORS_LIMIT_MESSAGE' => 'Μπορείτε να επιλέξετε έως και 10 στάδια', 
	'JS_KANBAN_NO_MORE_RECORDS' => 'Δεν βρέθηκαν άλλες εγγραφές',
	'JS_DELETED' => 'διαγράφηκε',
	'JS_CLEAR_TEMPLATE_CONTENT_RECIPIENTS'=>'Αν αλλάξετε την ενότητα, θα εκκαθαριστεί το επιλεγμένο πρότυπο και οι παραλήπτες από το πεδίο ΠΡΟΣ. Θέλετε να συνεχίσετε;',
	'JS_WORKFLOW_ACTIONS_LIMIT_EXCEEDED' => 'Υπέρβαση του μέγιστου ορίου ενεργειών σε αυτήν την έκδοση.',
	'JS_RECORD_CREATION_LIMIT_EXCEEDED' => 'Δημιουργία εγγραφής - Υπέρβαση του μέγιστου ορίου αυτής της έκδοσης.',
	'JS_KANBAN_NO_ANCHORS_CONFIGURED' => "Δεν έχετε οριστεί κανένα στάδιο πωλήσεων για τον pipeline πωλήσεων.",
	'JS_KANBAN_NO_ANCHORS_CONFIGURED_NON_ADMIN' => "Δεν έχει οριστεί κανένα στάδιο πωλήσεων για το pipeline πωλήσεων. Επικοινωνήστε με τον διαχειριστή σας για να ορίσετε τα στάδια πώλησης σύμφωνα με τη διαδικασία πώλησης.",
	'JS_UPDATE_MAPPING_CONFIRMATION' => 'Υπάρχουν ορισμένες αντίστοιχες στήλες στο αρχείο CSV που δεν αποθηκεύονται σε αυτή τη διασύνδεση. Θέλετε να ενημερώσετε αυτήν τη διασύνδεση με νέες στήλες;',
	'JS_MAPPING_UPDATED' => 'Ενημερώθηκε η νέα διασύνδεση.',
	'JS_MAILCHIMP_REMOVE_SYNC_CONFIRMATION' => 'Revoke access will remove existing MailChimp authentication, it will not delete synchronized records. Are you sure you want to revoke access?',
	'JS_WARNING' => 'Προειδοποίηση',
	'JS_CANT_SELECT_LEAD' => 'Δεν είναι δυνατή η επιλογή του σχετικού Δυνητικού',
	'JS_CANT_SELECT_CONTACT' => 'Δεν είναι δυνατή η επιλογή του σχετικού Πελάτη',
	'JS_MSG_EMPTY_RB_CONFIRMATION' => 'Είστε βέβαιοι ότι θέλετε να καταργήσετε οριστικά όλες τις διεγραμμένες εγγραφές από τη βάση δεδομένων σας;',
	'JS_LBL_RESTORE_RECORDS_CONFIRMATION' => 'Είστε βέβαιοι ότι θέλετε να επαναφέρετε τις εγγραφές;',
	'JS_COMMENT_POSTED_SUCCESSFULLY'=>'Το σχόλιο δημοσιεύτηκε με επιτυχία',
	'JS_SHARED_PUBLICLY_WITH'=>'Μοιράστηκε δημόσια με',
	'JS_SHARED_PRIVATELY_WITH'=>'Μοιράστηκε ιδιωτικά με',
	'JS_MAIL_SENT_SUCCESSFULLY'=>'Το email στάλθηκε με επιτυχία',
	'JS_SHARED_PUBLICLY_TITLE'=>'Η δημόσια συζήτηση μπορεί επίσης να προβληθεί από χρήστες εκτός των συνδεδεμένων γραμματοκιβωτίων, εάν έχουν πρόσβαση στον σχετικό πελάτη, την ευκαιρία ..κλπ',
	'JS_SHARED_PRIVATELY_TITLE'=>'Η ιδιωτική συνομιλία μπορεί να προβληθεί μόνο από μέλη των γραμματοκιβωτίων με τα οποία συνδέεται.',
	'JS_LINKED_RECORDS_SUCCESS'=>'Οι συνδεδεμένες εγγραφές ενημερώθηκαν με επιτυχία',
	'JS_MAIL_DRAFTED_SUCCESSFULLY'=>'Το email αποθηκεύτηκε με επιτυχία',

	//Custom view default sort translations.
	'JS_PLEASE_REMOVE_ONE_FIELD_FROM_CHOOSE_COLUMNS_LIST_TO_ADD_DEFAULT_SORT_FIELD' => 'Μόνο 16 στήλες επιτρέπονται σε μια προβολή λίστας που περιλαμβάνει στήλες ταξινόμησης.',
	'JS_DEFAULT_SORT_NOTIFY' => 'Η λίστα ταξινομείται τώρα στην προεπιλεγμένη στήλη ταξινόμησης ',
	'JS_ENABLED' => 'Ενεργοποιημένη',
	'JS_DISABLED' => 'Απενεργοποιημένη',
	'JS_ONLY_ALLOWED_USERS_ARE_ALLOWED_TO_DO_ACTION'=>'Δεν μπορέσαμε να επισημάνουμε τη συνομιλία ως μη αναγνωσμένη / αναγνωσμένη / διαγραμμένη / ολοκληρωμένη / ιδιωτική / δημόσια από τη στιγμή που δεν σας ανήκει.',
	'JS_SHARED_WITH'=>'Μοιρασμένο με',
	'JS_DEFAULT_DASHBOARD_TOOLTIP' => 'Ορίστε αυτόν τον πίνακα ελέγχου ως προεπιλογή, ανακατατάσσοντάς τον ως την πρώτη καρτέλα!',
	'JS_SET_DEFAULT_TAB' => 'ορίστηκε ως ο προεπιλεγμένος πίνακας ελέγχου',
	'JS_PASSWORD_MISMATCH_ERROR' => 'Καταχωρίστε ξανά τα συνθηματικά. Οι τιμές "νέο συνθηματικό" και "επιβεβαίωση συνθηματικού" δεν ταιριάζουν.',
	'LBL_LIST_DELETE_CONFIRMATION' => 'Είστε βέβαιοι ότι θέλετε να διαγράψετε;',
	'JS_WIDGET_RESIZING_WAIT_MSG' => 'Τα περιεχόμενα του widget θα φορτωθούν μετά την αλλαγή μεγέθους.',
);
